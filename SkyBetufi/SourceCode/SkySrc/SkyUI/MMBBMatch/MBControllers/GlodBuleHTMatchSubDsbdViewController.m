#import "GlodBuleHTMatchSubDsbdViewController.h"
#import "GlodBuleHTMatchDataLeftCell.h"
#import "GlodBuleHTMatchDataRightCell.h"
#import "GlodBulePlayerInfoViewController.h"

@interface GlodBuleHTMatchSubDsbdViewController ()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
...
@end
@implementation GlodBuleHTMatchSubDsbdViewController
...
- (void)setupLeftTableView {
    ...
    [self.leftTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GlodBuleHTMatchDataLeftCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([GlodBuleHTMatchDataLeftCell class])];
}
- (void)setupRightTableView {
    ...
    [self.rightTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GlodBuleHTMatchDataRightCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([GlodBuleHTMatchDataRightCell class])];
    [self.scrollContentView addSubview:self.rightTableView];
}
- (void)waterSkyrefreshWithDetailList:(NSArray<GlodBuleHTMatchDetailsModel *> *)detailList {
...
- (void)countWithDataList:(NSArray<GlodBuleHTMatchDetailsModel *> *)dataList { //计算总计
    if (dataList.count == 0) {
        return;
    }
    if (!self.counts) {
    ...
    NSInteger blk = 0;
    for (GlodBuleHTMatchDetailsModel *dmodel in dataList) {
    ...
    //添加一个命中率
    GlodBuleHTMatchDetailsModel *xxModel = [[GlodBuleHTMatchDetailsModel alloc] init];
    xxModel.name = @"命中率";
    xxModel.time = @"-";
    xxModel.pts = @"-";
    xxModel.ast = @"-";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    GlodBuleHTMatchDetailsModel *model = self.dataList[indexPath.row];
    if (tableView == self.leftTableView) {
        GlodBuleHTMatchDataLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([GlodBuleHTMatchDataLeftCell class])];
        [cell waterSkyrefreshWithName:model.name row:indexPath.row];
        return cell;
    }
    GlodBuleHTMatchDataRightCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([GlodBuleHTMatchDataRightCell class])];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.leftTableView) {
        ...
        GlodBulePlayerInfoViewController *piVc = [GlodBulePlayerInfoViewController waterSkyviewController];
        [piVc setData:model];
