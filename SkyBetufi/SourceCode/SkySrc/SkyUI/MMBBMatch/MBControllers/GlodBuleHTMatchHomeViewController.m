...
- (IBAction)onSelectDateButtonTapped:(id)sender {
    kWeakSelf
    [GlodBuleHTDatePickerView waterSkyshowWithWithDate:self.startDate didTapEnterBlock:^BOOL(NSDate *date) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    GlodBuleHTMatchHomeGroupModel *groupModel = self.matchList[indexPath.section];
    GlodBuleHTMatchDetailViewController *detailVc = [GlodBuleHTMatchDetailViewController waterSkyviewController];
...
- (void)loadData {
    ...
    self.requesting = YES;
    [GlodBuleHTMatchHomeRequest waterSkyrequestWithStartDate:[self ymdWithDate:self.startDate]
                                    ...
                                    if (matchList.count == 0) {
                                        ...
                                        if (self.inProgressMatchs.count == 0) {
                                            ...
                                            for (GlodBuleHTMatchHomeModel *model in self.inProgressMatchs.allValues) {
                                                [GlodBuleHTMatchHomeRequest waterSkyrequestMatchProgressWithGameId:model.game_id successBlock:^(NSString *game_id, NSString *quarter, NSString *time) {
