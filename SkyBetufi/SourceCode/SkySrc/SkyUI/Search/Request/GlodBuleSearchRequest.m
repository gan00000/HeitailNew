...
- (void)requestWithKey:(NSString *)key successBlock:(void(^)(NSArray<GlodBuleHTNewsModel *> *newsList))successBlock
    ...
    [GlodBuleBJHTTPServiceEngine waterSky_getRequestWithFunctionPath:API_SEARCH params:params successBlock:^(id responseData) {
...
- (void)loadNextPageWithSuccessBlock:(void(^)(NSArray<GlodBuleHTNewsModel *> *newsList))successBlock
    ...
    params[@"count"] = @20;
    params[@"page"] = @(self.page);
    [GlodBuleBJHTTPServiceEngine waterSky_getRequestWithFunctionPath:API_NEWS_HOME params:params successBlock:^(id responseData) {
        if (self.page < [responseData[@"pages"] integerValue]) {
            self.page ++;
            self.hasMore = YES;
        } else {
...
//                 errorBlock:(BJServiceErrorBlock)errorBlock {
//
//    [GlodBuleBJHTTPServiceEngine waterSky_getRequestWithFunctionPath:API_SEARCH params:@{@"search":key, @"page": [NSString stringWithFormat:@"%d",xPage], @"count":@20} successBlock:^(id responseData) {
//
