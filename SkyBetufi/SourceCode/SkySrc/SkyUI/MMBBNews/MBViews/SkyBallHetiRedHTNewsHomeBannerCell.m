...
@interface SkyBallHetiRedHTNewsHomeBannerCell () <UIScrollViewDelegate>
...
@end
@implementation SkyBallHetiRedHTNewsHomeBannerCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)waterSkysetupWithNewsModels:(NSArray *)bannerList {
    ...
    if (self.bannerList.count > 0) {
        for (int i = 0; i < self.bannerList.count; i++) {
            ...
            SkyBallHetiRedHTNewsModel *banner = self.bannerList[i];
        ...
        if (self.bannerList.count > 1) {
            ...
            [self.scrollView addSubview:imageView];
            SkyBallHetiRedHTNewsModel *lastBanner = [self.bannerList lastObject];
            ...
            imageView.tag = 0;
            [self.scrollView addSubview:imageView];
            SkyBallHetiRedHTNewsModel *firstBanner = [self.bannerList firstObject];
...
- (void)onBannerImageTapped:(UITapGestureRecognizer *)tap {
    ...
    SkyBallHetiRedHTNewsModel *banner = self.bannerList[tapImageView.tag];
...
- (void)scrollBanner {
    ...
    [self.scrollView setContentOffset:CGPointMake((currentIndex+1)*SCREEN_WIDTH, 0)
                             animated:YES];
    SkyBallHetiRedHTNewsModel *nextModel;
...
- (void)p_setupTitle {
    ...
    SkyBallHetiRedHTNewsModel *currentModel = self.bannerList[index];
