...
- (void)refreshWithCommentModel:(XRRFATKHTCommentModel *)commentModel {
    ...
    self.timelabel.text = [XRRFATKBJDateFormatUtility dateToShowFromDate:commentModel.date];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        ...
        indexPath.row < 3) {
        XRRFATKHTNewsCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self class])];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        (self.commentModel.expend && indexPath.row < self.commentModel.reply.count) ||
        indexPath.row < 3) {
        XRRFATKHTCommentModel *model = self.commentModel.reply[indexPath.row];
