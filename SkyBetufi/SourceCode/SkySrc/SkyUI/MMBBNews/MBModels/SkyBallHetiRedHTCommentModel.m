...
@interface SkyBallHetiRedHTCommentModel ()
...
@end
@implementation SkyBallHetiRedHTCommentModel
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"like": [SkyBallHetiRedHTUserInfoModel class],
             @"reply": [SkyBallHetiRedHTCommentModel class]
             };
}
- (void)waterSkycountHeight:(BOOL)isReply {
    ...
    if (!isReply) {
        ...
        for (NSInteger i=0; i<self.reply.count; i++) {
            SkyBallHetiRedHTCommentModel *model = self.reply[i];
...
- (SkyBallHetiRedHTUserInfoModel *)userModel {
    if (!_userModel) {
        _userModel = [[SkyBallHetiRedHTUserInfoModel alloc] init];
