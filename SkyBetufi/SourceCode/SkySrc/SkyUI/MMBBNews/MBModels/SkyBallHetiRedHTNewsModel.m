...
@interface SkyBallHetiRedHTNewsModel () <NSURLConnectionDelegate>
@property (nonatomic, copy) NSString *clearContent;
@end
@implementation SkyBallHetiRedHTNewsModel
...
- (void)setContent:(NSString *)content {
    ...
    if (iframe) {
        ...
        NSLog(@"iframe = %@", iframe);
        _iframe = [[SkyBallHetiRedHTHtmlLoadUtil sharedInstance] iframHtmlWithContent:iframe];
        NSLog(@"iframe = %@", _iframe);
    } 
}
- (void)setDate:(NSString *)date {
    _date = date;
    _time = [SkyBallHetiRedBJDateFormatUtility dateToShowFromDateString:date];
}
...
- (void)waterSkyshare {
    kWeakSelf
    [SkyBallHetiRedHTLoginAlertView waterSkyshowShareAlertViewWithSelectBlock:^(HTLoginPlatform platform) {
        ...
        if (platform == HTLoginPlatformFB) {
            ...
            if (weakSelf.img_url) {
                [SkyBallHetiRedBJLoadingHud showHUDInView:[SkyBallHetiRedPPXXBJViewControllerCenter currentViewController].view];
                [[SDWebImageManager sharedManager] loadImageWithURL:[NSURL URLWithString:weakSelf.img_url] options:0 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                    ...
                    [SkyBallHetiRedBJLoadingHud hideHUDInView:[SkyBallHetiRedPPXXBJViewControllerCenter currentViewController].view];
                }];
            } else {
                [weakSelf doShareToPlatform:UMSocialPlatformType_Line withMessage:messageObject];
            }
...
- (SkyBallHetiRedHTUserInfoModel *)userInfo {
    if (!_userInfo) {
        _userInfo = [[SkyBallHetiRedHTUserInfoModel alloc] init];
