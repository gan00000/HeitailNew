...
@end
@implementation GlodBuleHTNewsModel
...
- (void)setContent:(NSString *)content {
    ...
    if (iframe) {
        ...
        NSLog(@"iframe = %@", iframe);
        _iframe = [[GlodBuleHTHtmlLoadUtil sharedInstance] iframHtmlWithContent:iframe];
        NSLog(@"iframe = %@", _iframe);
    } 
}
- (void)setDate:(NSString *)date {
    _date = date;
    _time = [GlodBuleBJDateFormatUtility dateToShowFromDateString:date];
}
...
- (GlodBuleHTUserInfoModel *)userInfo {
    if (!_userInfo) {
        _userInfo = [[GlodBuleHTUserInfoModel alloc] init];
