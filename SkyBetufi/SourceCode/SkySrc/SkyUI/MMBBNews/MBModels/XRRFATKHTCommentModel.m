...
@interface XRRFATKHTCommentModel ()
...
@implementation XRRFATKHTCommentModel

+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"like": [XRRFATKHTUserInfoModel class],
             @"reply": [XRRFATKHTCommentModel class]
             };
}
...
- (void)countHeight:(BOOL)isReply {
    ...
    if (!isReply) {
        ...
        for (NSInteger i=0; i<self.reply.count; i++) {
            XRRFATKHTCommentModel *model = self.reply[i];
...
- (XRRFATKHTUserInfoModel *)userModel {
    if (!_userModel) {
        _userModel = [[XRRFATKHTUserInfoModel alloc] init];
