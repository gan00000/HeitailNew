...
@interface XRRFATKHTNewsHomeOtherViewController () <UIScrollViewDelegate>
...
@end
@implementation XRRFATKHTNewsHomeOtherViewController
+ (instancetype)skargviewController {
    return kLoadStoryboardWithName(@"XRRFATKNewsHomeOther");
}
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initData];
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    UIViewController *vc;
    if (index == 0) {
        XRRFATKHTNewsHomeViewController *ewVc = [XRRFATKHTNewsHomeViewController skargviewController];
        vc = ewVc;
    } else if (index == 1) {
        XRRFATKHTFilmHomeViewController *zoneVc = [XRRFATKHTFilmHomeViewController skargviewController];
        vc = zoneVc;
    }
...
- (HMSegmentedControl *)segmentControl {
    if (!_segmentControl) {
        _segmentControl = [[HMSegmentedControl alloc] initWithSectionTitles:@[@"熱門", @"影片"]];
