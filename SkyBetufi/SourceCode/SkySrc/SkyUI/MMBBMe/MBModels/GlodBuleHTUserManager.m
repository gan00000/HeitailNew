...
@interface GlodBuleHTUserManager () <LineSDKLoginDelegate,ASAuthorizationControllerDelegate,ASAuthorizationControllerPresentationContextProviding>
@property (nonatomic, strong) GlodBuleHTUserInfoModel *userInfoModel;
@property (nonatomic, strong) FBSDKLoginManager *waterSky_fbLoginManager;
@end
@implementation GlodBuleHTUserManager
- (instancetype)init {
    self = [super init];
    if (self) {
        self.svgImageCache = [[NSMutableDictionary alloc] initWithCapacity:0];
        if ([GlodBuleHTUserManager waterSky_isUserLogin]) {
            NSData *data = [NSData dataWithContentsOfFile:[GlodBuleHTUserManager waterSkyuserInfoPath]];
            self.userInfoModel = [GlodBuleHTUserInfoModel yy_modelWithJSON:data];
        }
    }
    return self;
}
...
+ (instancetype)manager {
    static GlodBuleHTUserManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[GlodBuleHTUserManager alloc] init];
    });
    return manager;
}
+ (BOOL)waterSky_isUserLogin {
    return [GlodBuleHTUserManager waterSky_userToken].length > 0;
}
+ (GlodBuleHTUserInfoModel *)waterSky_userInfo {
    return [GlodBuleHTUserManager manager].userInfoModel;
}
+ (void)saveUserInfo:(NSDictionary *)userInfo {
    GlodBuleHTUserManager *manager = [GlodBuleHTUserManager manager];
    manager.userInfoModel = [GlodBuleHTUserInfoModel yy_modelWithJSON:userInfo];
    NSData *data = [userInfo yy_modelToJSONData];
    [data writeToFile:[GlodBuleHTUserManager waterSkyuserInfoPath] atomically:YES];
...
+ (void)waterSky_doUserLogin {
    [GlodBuleHTLoginAlertView waterSkyshowLoginAlertViewWithSelectBlock:^(HTLoginPlatform platform) {
        GlodBuleHTUserManager *manager = [GlodBuleHTUserManager manager];
...
+ (void)waterSky_doUserLogout {
    ...
    [GlodBuleDRSandBoxManager waterSkydeleteFileAtPath:[self waterSkyuserInfoPath] doneBlock:^(NSString * _Nonnull filePath, BOOL success, NSError * _Nonnull error) {
        BJLog(@"用戶信息刪除 %d", success);
    }];
    [[NSNotificationCenter defaultCenter] postNotificationName:kUserLogStatusChagneNotice object:nil];
    [[GlodBuleHTUserManager manager].waterSky_fbLoginManager logOut];
}
#pragma mark - Facebook Authory
- (void)getAuthWithUserInfoFromFacebook {
    kWeakSelf
    [self.waterSky_fbLoginManager logInWithPermissions:@[@"public_profile",@"email"] fromViewController:[GlodBulePPXXBJViewControllerCenter currentViewController] handler:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
            if (error) {
...
//    [self.waterSky_fbLoginManager logInWithReadPermissions: @[@"public_profile",@"email"] fromViewController:[GlodBulePPXXBJViewControllerCenter currentViewController] handler:^(FBSDKLoginManagerLoginResult *result, NSError *error) {
//         if (error) {
//             [kWindow showToast:@"登錄失敗"];
//         } else if (result.isCancelled) {
//             [kWindow showToast:@"取消登錄"];
...
- (void)doLoginRequesWithAccessToken:(NSString *)accessToken sns:(NSInteger)sns {
    [GlodBuleHTUserRequest waterSkydoLoginRequestWithAccessToken:accessToken sns:sns successBlock:^(NSString * _Nonnull userToken) {
        [GlodBuleHTUserManager saveUserToken:userToken];
        [GlodBuleHTUserManager waterSky_refreshUserInfoWithSuccessBlock:nil];
...
+ (void)waterSky_refreshUserInfoWithSuccessBlock:(dispatch_block_t)block {
    [GlodBuleHTUserRequest waterSkyrequestUserInfoWithSuccessBlock:^(NSDictionary * _Nonnull userInfo) {
...
- (ASPresentationAnchor)presentationAnchorForAuthorizationController:(ASAuthorizationController *)controller API_AVAILABLE(ios(13.0)){
      ...
      // 返回window
      return [GlodBulePPXXBJViewControllerCenter currentViewController].view.window;
}

#pragma mark - util
+ (NSString *)waterSkyuserInfoPath {
    return [NSString stringWithFormat:@"%@/userInfo.json", [GlodBuleDRSandBoxManager waterSkygetDocumentPath]];
}
...
+ (void)waterSky_showAlertWithTitle:(NSString *)title message:(NSString *)message cancelButton:(NSString *)cancelButton confirmButton:(NSString *)confirmButton confirmBlock:(dispatch_block_t)confirmBlock {
    ...
    [[GlodBulePPXXBJViewControllerCenter currentViewController] presentViewController:alert animated:YES completion:nil];
}
