...
- (void)showEmptyViewWithImage:(UIImage *)image title:(NSString *)title tapBlock:(dispatch_block_t)tapBlock edge:(UIEdgeInsets)edge {
    [self hideEmptyView];
    SkyBallHetiRedXJEmptyView *emptyView = [[NSBundle mainBundle] loadNibNamed:@"SkyBallHetiRedXJEmptyView" owner:nil options:nil].firstObject;
    if (title.length > 0) {
