...
@interface GlodBuleConfigCoreUtil()


@end

@implementation GlodBuleConfigCoreUtil

static GlodBuleConfigCoreUtil * configUtil;
...
+ (GlodBuleConfigCoreUtil *)share
{
    ...
    dispatch_once(&onceToken,^{
        configUtil = [[GlodBuleConfigCoreUtil alloc] init];
    });
    return configUtil;
}
...
-(void)saveAccount:(NSString *) mAccount password:(NSString *) password updateTime:(BOOL) updateTime
{
    ...
    for (GlodBuleAccountModel *am in mAccountArray) {
        if ([am.accountName isEqualToString:mAccount]) {
            am.accountPwd = password;
            if (updateTime) {
                am.lastLoginTime = [GlodBuleConfigCoreUtil getTimeStamp];
            }
    ...
    GlodBuleAccountModel *mAccountModel = [[GlodBuleAccountModel alloc] init];
    //赋值
    mAccountModel.lastLoginTime = [GlodBuleConfigCoreUtil getTimeStamp];
...
-(void)removeAccount:(NSString *) mAccount
{
    ...
    for (GlodBuleAccountModel *am in dataList) {
        if ([am.accountName isEqualToString:mAccount]) {
...
-(void)saveAccountModels:(NSArray<GlodBuleAccountModel *> *) mAccountModelArray
{
    ...
    for (GlodBuleAccountModel *m in mAccountModelArray) {
        //存储到NSUserDefaults
