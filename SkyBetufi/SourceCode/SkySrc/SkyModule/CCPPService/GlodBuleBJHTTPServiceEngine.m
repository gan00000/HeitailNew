...
+ (void)waterSky_getRequestCommon:(NSString *)path
    ...
    [[GlodBuleBJHTTPServiceEngine sharedInstance].httpEngine waterSky_getRequestWithFunctionPath:path params:allParams successBlock:^(NSURLSessionDataTask *task, id responseData) {
...
+ (void)waterSky_getRequestWithFunctionPath:(NSString *)path
    ...
    if ([GlodBuleHTUserManager waterSky_isUserLogin]) {
        allParams[@"token"] = [GlodBuleHTUserManager waterSky_userToken];
    }
    if (params) {
        [allParams addEntriesFromDictionary:params];
    }
...
+ (void)waterSky_postRequestWithFunctionPath:(NSString *)path
    ...
    [[GlodBuleBJHTTPServiceEngine sharedInstance].httpEngine waterSky_postRequestWithFunctionPath:path params:allParams successBlock:^(NSURLSessionDataTask *task, id responseData) {
#if ENABLE_REQUEST_LOG
...
+ (void)waterSky_fileUploadWithFunctionPath:(NSString *)functionPath
                     ...
                     progressBlock:(BJHTTPProgressBlock)progressBlock
                      successBlock:(BJServiceSuccessBlock)successBlock
                        errorBlock:(BJServiceErrorBlock)errorBlock {
    [[GlodBuleBJHTTPServiceEngine sharedInstance].httpEngine waterSky_fileUploadWithFunctionPath:functionPath params:params fileData:fileData fileName:fileName mimeType:mimeType progressBlock:^(float progress) {
        if (progressBlock) {
...
+ (void)waterSky_imageUploadWithFunctionPath:(NSString *)functionPath
                      ...
                      progressBlock:(BJHTTPProgressBlock)progressBlock
                       successBlock:(BJServiceSuccessBlock)successBlock
                         errorBlock:(BJServiceErrorBlock)errorBlock {
    [[GlodBuleBJHTTPServiceEngine sharedInstance].httpEngine waterSky_imageUploadWithFunctionPath:functionPath params:params imageData:imageData imageName:imageName progressBlock:^(float progress) {
        if (progressBlock) {
            progressBlock(progress);
        }
    } successBlock:^(NSURLSessionDataTask *task, id responseData) {
