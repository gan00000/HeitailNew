//
//  SkyBallHetiRedDHGuidePageHUD.m
//  SkyBallHetiRedDHGuidePageHUD
//
...
@interface SkyBallHetiRedDHGuidePageHUD ()<UIScrollViewDelegate>
@property (nonatomic, strong) NSArray                 *imageArray;
@property (nonatomic, strong) UIPageControl           *imagePageControl;
@property (nonatomic, assign) NSInteger               slideIntoNumber;
@property (nonatomic, strong) MPMoviePlayerController *playerController;

@property (nonatomic,strong) ClickListener mClickListener;
@end

@implementation SkyBallHetiRedDHGuidePageHUD
...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if ([[SkyBallHetiRedDHGifImageOperation dh_contentTypeForImageData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageNameArray[i] ofType:nil]]] isEqualToString:@"gif"]) {
                NSData *localData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageNameArray[i] ofType:nil]];
                imageView = (UIImageView *)[[SkyBallHetiRedDHGifImageOperation alloc] initWithFrame:imageView.frame gifImageData:localData];
...
- (void)buttonClick:(UIButton *)button {
    ...
    if (self.mClickListener) {
        self.mClickListener(0);
    }
}
