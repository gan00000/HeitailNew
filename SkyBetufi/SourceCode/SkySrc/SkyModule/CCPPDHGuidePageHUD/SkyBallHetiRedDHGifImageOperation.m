//
//  SkyBallHetiRedDHGifImageOperation.m
...
@implementation SkyBallHetiRedDHGifImageOperation
...
- (void)play {
    if (count > 0) {
        ...
        dispatch_once(&onceToken, ^{
            NSLog(@"[SkyBallHetiRedDHGifImageOperation]:请检测网络或者http协议");
        });
    }
}
...
/**< 使用案例: [self.XXX addSubview:[[SkyBallHetiRedDHGifImageOperation alloc] initWithFrame:self.adFrame gifImageName:@"XXX.gif"]]; */
...
- (void)activiTap:(UITapGestureRecognizer*)recognizer{
    NSLog(@"[SkyBallHetiRedDHGifImageOperation.h]:activiTap:recognizer");
}
@end
