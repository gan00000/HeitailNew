...
#import "GlodBuleTZPhotoPreviewController.h"
#import "GlodBuleTZAssetCell.h"
#import "GlodBuleTZAssetModel.h"
#import "UIView+GlodBuleLayout.h"
#import "GlodBuleTZImageManager.h"
#import "GlodBuleTZVideoPlayerController.h"
#import "GlodBuleTZGifPhotoPreviewController.h"
#import "GlodBuleTZLocationManager.h"
@interface GlodBuleTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation GlodBuleTZPhotoPickerController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    GlodBuleTZImagePickerController *tzImagePickerVc = (GlodBuleTZImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            [[GlodBuleTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
                _model = model;
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[GlodBuleTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<GlodBuleTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[GlodBuleTZAssetCell class] forCellWithReuseIdentifier:@"GlodBuleTZAssetCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    [GlodBuleTZImageManager manager].columnNumber = [GlodBuleTZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    GlodBuleTZPhotoPreviewController *photoPreviewVc = [[GlodBuleTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    [GlodBuleTZImageManager manager].shouldFixOrientation = YES;
    __block id alertView;
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        GlodBuleTZAssetModel *model = tzImagePickerVc.selectedModels[i];
        [[GlodBuleTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GlodBuleTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GlodBuleTZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        GlodBuleTZImagePickerController *tzImagePickerVc = (GlodBuleTZImagePickerController *)weakSelf.navigationController;
        if (isSelected) {
            weakCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (GlodBuleTZAssetModel *model_item in selectedModels) {
                if ([[[GlodBuleTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[GlodBuleTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GlodBuleTZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            GlodBuleTZImagePickerController *imagePickerVc = (GlodBuleTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            GlodBuleTZVideoPlayerController *videoPlayerVc = [[GlodBuleTZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            GlodBuleTZGifPhotoPreviewController *gifPreviewVc = [[GlodBuleTZGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    __weak typeof(self) weakSelf = self;
    [[GlodBuleTZLocationManager manager] startLocationWithSuccessBlock:^(CLLocation *location, CLLocation *oldLocation) {
...
- (void)pushPhotoPrevireViewController:(GlodBuleTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    GlodBuleTZImagePickerController *imagePickerVc = (GlodBuleTZImagePickerController *)self.navigationController;
    [[GlodBuleTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (GlodBuleTZAssetModel *model in _models) {
        ...
        GlodBuleTZImagePickerController *tzImagePickerVc = (GlodBuleTZImagePickerController *)self.navigationController;
        for (GlodBuleTZAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[GlodBuleTZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
...
- (void)reloadPhotoArray {
    ...
    [[GlodBuleTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
        ...
        [[GlodBuleTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<GlodBuleTZAssetModel *> *models) {
            [tzImagePickerVc hideProgressHUD];
            GlodBuleTZAssetModel *assetModel;
            if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (void)resetCachedAssets {
    [[GlodBuleTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[GlodBuleTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[GlodBuleTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            GlodBuleTZAssetModel *model = _models[indexPath.item];
