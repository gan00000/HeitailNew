...
@interface SkyBallHetiRedTZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
@end
@implementation SkyBallHetiRedTZImagePickerController
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[SkyBallHetiRedTZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[SkyBallHetiRedTZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[SkyBallHetiRedTZImageManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    SkyBallHetiRedTZPhotoPreviewController *previewVc = [[SkyBallHetiRedTZPhotoPreviewController alloc] init];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        SkyBallHetiRedTZPhotoPickerController *photoPickerVc = [[SkyBallHetiRedTZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[SkyBallHetiRedTZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(TZAlbumModel *model) {
...
- (void)setPickerDelegate:(id<TZImagePickerControllerDelegate>)pickerDelegate {
    ...
    [SkyBallHetiRedTZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [SkyBallHetiRedTZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}
- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [SkyBallHetiRedTZImageManager manager].photoWidth = photoWidth;
}
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    _selectedAssets = selectedAssets;
    _selectedModels = [NSMutableArray array];
    for (id asset in selectedAssets) {
        SkyBallHetiRedTZAssetModel *model = [SkyBallHetiRedTZAssetModel modelWithAsset:asset type:[[SkyBallHetiRedTZImageManager manager] getAssetType:asset]];
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [SkyBallHetiRedTZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)configTableView {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        SkyBallHetiRedTZImagePickerController *imagePickerVc = (SkyBallHetiRedTZImagePickerController *)self.navigationController;
        [[SkyBallHetiRedTZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
