...
#import "SkyBallHetiRedDRDeleteAlertView.h"
@interface SkyBallHetiRedTZVideoPlayerController () {
    AVPlayer *_player;
...
@implementation SkyBallHetiRedTZVideoPlayerController
...
- (void)configMoviePlayer {
    if (self.coverImg) {
        ...
        [[SkyBallHetiRedTZImageManager manager] getOriginalPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info) {
            _cover = photo;
        }];
    }
    if (self.videoPath.length > 0) {
        ...
        kWeakSelf
        [[SkyBallHetiRedTZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)delectButtonClick {
    [SkyBallHetiRedDRDeleteAlertView showDeleteAlertViewWithComplete:^(BOOL isDelete) {
        if (isDelete) {
