...
#import "SkyBallHetiRedTZProgressView.h"
@interface SkyBallHetiRedTZAssetCell ()
@property (weak, nonatomic) UIImageView *imageView;       
@property (weak, nonatomic) UIImageView *selectImageView;
@property (weak, nonatomic) UIView *bottomView;
@property (weak, nonatomic) UILabel *timeLength;
@property (nonatomic, weak) UIImageView *videoImgView;
@property (nonatomic, strong) SkyBallHetiRedTZProgressView *progressView;
@property (nonatomic, assign) int32_t bigImageRequestID;
@end
@implementation SkyBallHetiRedTZAssetCell
- (void)setModel:(SkyBallHetiRedTZAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[SkyBallHetiRedTZImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[SkyBallHetiRedTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[SkyBallHetiRedTZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)fetchBigImage {
    _bigImageRequestID = [[SkyBallHetiRedTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (SkyBallHetiRedTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[SkyBallHetiRedTZProgressView alloc] init];
...
- (void)setModel:(TZAlbumModel *)model {
    ...
    [[SkyBallHetiRedTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
