...
@implementation XRRFATKTZAssetCell

- (void)setModel:(XRRFATKTZAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[XRRFATKTZImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[XRRFATKTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[XRRFATKTZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)fetchBigImage {
    _bigImageRequestID = [[XRRFATKTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (XRRFATKTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[XRRFATKTZProgressView alloc] init];
...
- (void)setModel:(TZAlbumModel *)model {
    ...
    [[XRRFATKTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
