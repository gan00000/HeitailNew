...
#import "SkyBallHetiRedTZPhotoPreviewController.h"
#import "SkyBallHetiRedTZAssetCell.h"
#import "SkyBallHetiRedTZAssetModel.h"
#import "UIView+SkyBallHetiRedLayout.h"
#import "SkyBallHetiRedTZImageManager.h"
#import "SkyBallHetiRedTZVideoPlayerController.h"
#import "SkyBallHetiRedTZGifPhotoPreviewController.h"
#import "SkyBallHetiRedTZLocationManager.h"
@interface SkyBallHetiRedTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation SkyBallHetiRedTZPhotoPickerController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    SkyBallHetiRedTZImagePickerController *tzImagePickerVc = (SkyBallHetiRedTZImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            [[SkyBallHetiRedTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
                _model = model;
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[SkyBallHetiRedTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<SkyBallHetiRedTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[SkyBallHetiRedTZAssetCell class] forCellWithReuseIdentifier:@"SkyBallHetiRedTZAssetCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    [SkyBallHetiRedTZImageManager manager].columnNumber = [SkyBallHetiRedTZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    SkyBallHetiRedTZPhotoPreviewController *photoPreviewVc = [[SkyBallHetiRedTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    [SkyBallHetiRedTZImageManager manager].shouldFixOrientation = YES;
    __block id alertView;
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        SkyBallHetiRedTZAssetModel *model = tzImagePickerVc.selectedModels[i];
        [[SkyBallHetiRedTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SkyBallHetiRedTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"SkyBallHetiRedTZAssetCell" forIndexPath:indexPath];
    ...
    SkyBallHetiRedTZAssetModel *model;
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        SkyBallHetiRedTZImagePickerController *tzImagePickerVc = (SkyBallHetiRedTZImagePickerController *)weakSelf.navigationController;
        if (isSelected) {
            weakCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (SkyBallHetiRedTZAssetModel *model_item in selectedModels) {
                if ([[[SkyBallHetiRedTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[SkyBallHetiRedTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SkyBallHetiRedTZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            SkyBallHetiRedTZImagePickerController *imagePickerVc = (SkyBallHetiRedTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            SkyBallHetiRedTZVideoPlayerController *videoPlayerVc = [[SkyBallHetiRedTZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            SkyBallHetiRedTZGifPhotoPreviewController *gifPreviewVc = [[SkyBallHetiRedTZGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    __weak typeof(self) weakSelf = self;
    [[SkyBallHetiRedTZLocationManager manager] startLocationWithSuccessBlock:^(CLLocation *location, CLLocation *oldLocation) {
...
- (void)pushPhotoPrevireViewController:(SkyBallHetiRedTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    SkyBallHetiRedTZImagePickerController *imagePickerVc = (SkyBallHetiRedTZImagePickerController *)self.navigationController;
    [[SkyBallHetiRedTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (SkyBallHetiRedTZAssetModel *model in _models) {
        ...
        SkyBallHetiRedTZImagePickerController *tzImagePickerVc = (SkyBallHetiRedTZImagePickerController *)self.navigationController;
        for (SkyBallHetiRedTZAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[SkyBallHetiRedTZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
...
- (void)reloadPhotoArray {
    ...
    [[SkyBallHetiRedTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
        ...
        [[SkyBallHetiRedTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<SkyBallHetiRedTZAssetModel *> *models) {
            [tzImagePickerVc hideProgressHUD];
            SkyBallHetiRedTZAssetModel *assetModel;
            if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (void)resetCachedAssets {
    [[SkyBallHetiRedTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[SkyBallHetiRedTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[SkyBallHetiRedTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            SkyBallHetiRedTZAssetModel *model = _models[indexPath.item];
