//
//  XRRFATKTZPhotoPickerController.m
...
@interface XRRFATKTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation XRRFATKTZPhotoPickerController
...
//        if (iOS9Later) {
//            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[XRRFATKTZImagePickerController class]]];
//            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
//        } else {
//            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[XRRFATKTZImagePickerController class], nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    XRRFATKTZImagePickerController *tzImagePickerVc = (XRRFATKTZImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            [[XRRFATKTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
                _model = model;
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[XRRFATKTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<XRRFATKTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[XRRFATKTZAssetCell class] forCellWithReuseIdentifier:@"XRRFATKTZAssetCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    [XRRFATKTZImageManager manager].columnNumber = [XRRFATKTZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    XRRFATKTZPhotoPreviewController *photoPreviewVc = [[XRRFATKTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    [XRRFATKTZImageManager manager].shouldFixOrientation = YES;
    __block id alertView;
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        XRRFATKTZAssetModel *model = tzImagePickerVc.selectedModels[i];
        [[XRRFATKTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XRRFATKTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"XRRFATKTZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        XRRFATKTZImagePickerController *tzImagePickerVc = (XRRFATKTZImagePickerController *)weakSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            weakCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (XRRFATKTZAssetModel *model_item in selectedModels) {
                if ([[[XRRFATKTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[XRRFATKTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XRRFATKTZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            XRRFATKTZVideoPlayerController *videoPlayerVc = [[XRRFATKTZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            XRRFATKTZGifPhotoPreviewController *gifPreviewVc = [[XRRFATKTZGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    // 提前定位
    __weak typeof(self) weakSelf = self;
    [[XRRFATKTZLocationManager manager] startLocationWithSuccessBlock:^(CLLocation *location, CLLocation *oldLocation) {
...
- (void)pushPhotoPrevireViewController:(XRRFATKTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    XRRFATKTZImagePickerController *imagePickerVc = (XRRFATKTZImagePickerController *)self.navigationController;
    [[XRRFATKTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (XRRFATKTZAssetModel *model in _models) {
        ...
        XRRFATKTZImagePickerController *tzImagePickerVc = (XRRFATKTZImagePickerController *)self.navigationController;
        for (XRRFATKTZAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[XRRFATKTZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
...
- (void)reloadPhotoArray {
    ...
    [[XRRFATKTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
        ...
        [[XRRFATKTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<XRRFATKTZAssetModel *> *models) {
            ...
            XRRFATKTZAssetModel *assetModel;
            if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (void)resetCachedAssets {
    [[XRRFATKTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[XRRFATKTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[XRRFATKTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            XRRFATKTZAssetModel *model = _models[indexPath.item];
