...
@interface KMonkeyDHGifImageOperation ()
{
...
@implementation KMonkeyDHGifImageOperation
...
- (void)play {
    if (count > 0) {
        ...
        dispatch_once(&onceToken, ^{
            NSLog(@"[KMonkeyDHGifImageOperation]:请检测网络或者http协议");
        });
    }
}
...
/**< 使用案例: [self.XXX addSubview:[[KMonkeyDHGifImageOperation alloc] initWithFrame:self.adFrame gifImageName:@"XXX.gif"]]; */
...
- (void)activiTap:(UITapGestureRecognizer*)recognizer{
    NSLog(@"[KMonkeyDHGifImageOperation.h]:activiTap:recognizer");
}
@end
