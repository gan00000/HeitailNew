...
#import "BByasDHGifImageOperation.h"
...
@interface BByasDHGuidePageHUD ()<UIScrollViewDelegate>
...
@implementation BByasDHGuidePageHUD
...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if ([[BByasDHGifImageOperation dh_contentTypeForImageData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageNameArray[i] ofType:nil]]] isEqualToString:@"gif"]) {
                NSData *localData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageNameArray[i] ofType:nil]];
                imageView = (UIImageView *)[[BByasDHGifImageOperation alloc] initWithFrame:imageView.frame gifImageData:localData];
