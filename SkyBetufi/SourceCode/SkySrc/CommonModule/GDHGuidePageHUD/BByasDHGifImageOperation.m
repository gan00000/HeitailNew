...
@interface BByasDHGifImageOperation ()
{
...
@implementation BByasDHGifImageOperation
...
- (void)play {
    if (count > 0) {
        ...
        dispatch_once(&onceToken, ^{
            NSLog(@"[BByasDHGifImageOperation]:请检测网络或者http协议");
        });
    }
}
...
/**< 使用案例: [self.XXX addSubview:[[BByasDHGifImageOperation alloc] initWithFrame:self.adFrame gifImageName:@"XXX.gif"]]; */
...
- (void)activiTap:(UITapGestureRecognizer*)recognizer{
    NSLog(@"[BByasDHGifImageOperation.h]:activiTap:recognizer");
}
@end
