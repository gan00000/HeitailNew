...
@interface GlodBuleDHGifImageOperation ()
{
...
@implementation GlodBuleDHGifImageOperation
...
- (void)play {
    if (count > 0) {
        ...
        dispatch_once(&onceToken, ^{
            NSLog(@"[GlodBuleDHGifImageOperation]:请检测网络或者http协议");
        });
    }
}
...
/**< 使用案例: [self.XXX addSubview:[[GlodBuleDHGifImageOperation alloc] initWithFrame:self.adFrame gifImageName:@"XXX.gif"]]; */
...
- (void)activiTap:(UITapGestureRecognizer*)recognizer{
    NSLog(@"[GlodBuleDHGifImageOperation.h]:activiTap:recognizer");
}
@end
