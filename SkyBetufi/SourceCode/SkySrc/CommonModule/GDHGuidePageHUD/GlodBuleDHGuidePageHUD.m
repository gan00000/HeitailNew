//
//  GlodBuleDHGuidePageHUD.m
//  GlodBuleDHGuidePageHUD
//
...
#import "GlodBuleDHGifImageOperation.h"
...
@interface GlodBuleDHGuidePageHUD ()<UIScrollViewDelegate>
...
@implementation GlodBuleDHGuidePageHUD
...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if ([[GlodBuleDHGifImageOperation dh_contentTypeForImageData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageNameArray[i] ofType:nil]]] isEqualToString:@"gif"]) {
                NSData *localData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageNameArray[i] ofType:nil]];
                imageView = (UIImageView *)[[GlodBuleDHGifImageOperation alloc] initWithFrame:imageView.frame gifImageData:localData];
