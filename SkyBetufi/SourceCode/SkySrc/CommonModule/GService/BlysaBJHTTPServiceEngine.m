...
#import "KSasxBJServiceConfigurator.h"
#import "YYModel.h"
#import "AFHTTPSessionManager.h"
#import "UUaksBJUtility.h"
#import "FFlaliBJBaseResponceModel.h"
@interface BlysaBJHTTPServiceEngine ()
@property (nonatomic, strong) TuTuosBJBaseHTTPEngine *httpEngine;
@end
@implementation BlysaBJHTTPServiceEngine
+ (instancetype)sharedInstance {
    static BlysaBJHTTPServiceEngine *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[BlysaBJHTTPServiceEngine alloc] init];
        NSString *servicePath = [[KSasxBJServiceConfigurator sharedInstance] serverBaseUrl];
        ...
        [instance.httpEngine tao_updateSessionWithBlock:^(AFHTTPSessionManager *session) {
            ...
            [session.requestSerializer setValue:[NSString stringWithFormat:@"%@|%@|%@", [UUaksBJUtility modelName], [UUaksBJUtility systemVersion], [UUaksBJUtility idfa]] forHTTPHeaderField:@"X-User-Device"]; 
            [session.requestSerializer setValue:[UUaksBJUtility appVersion] forHTTPHeaderField:@"X-App-Version"];
        }];
    });
    return instance;
}
...
+ (void)tao_getRequestCommon:(NSString *)path
    ...
    [[BlysaBJHTTPServiceEngine sharedInstance].httpEngine tao_getRequestWithFunctionPath:path params:allParams successBlock:^(NSURLSessionDataTask *task, id responseData) {
        ...
        if (responseData) {
            ...
            YeYeeBJError *errorObject = [YeYeeBJError yy_modelWithDictionary:responseDict];
            if (errorBlock) {
        ...
        if (errorBlock) {
            YeYeeBJError *errorObject = [[YeYeeBJError alloc] init];
...
+ (void)tao_getRequestWithFunctionPath:(NSString *)path
    ...
    if ([NSNiceHTUserManager tao_isUserLogin]) {
        allParams[@"token"] = [NSNiceHTUserManager tao_userToken];
    }
    if (params) {
        [allParams addEntriesFromDictionary:params];
    }
...
+ (void)tao_postRequestWithFunctionPath:(NSString *)path
    ...
    [[BlysaBJHTTPServiceEngine sharedInstance].httpEngine tao_postRequestWithFunctionPath:path params:allParams successBlock:^(NSURLSessionDataTask *task, id responseData) {
#if ENABLE_REQUEST_LOG
...
+ (void)tao_fileUploadWithFunctionPath:(NSString *)functionPath
                     ...
                     progressBlock:(BJHTTPProgressBlock)progressBlock
                      successBlock:(BJServiceSuccessBlock)successBlock
                        errorBlock:(BJServiceErrorBlock)errorBlock {
    [[BlysaBJHTTPServiceEngine sharedInstance].httpEngine tao_fileUploadWithFunctionPath:functionPath params:params fileData:fileData fileName:fileName mimeType:mimeType progressBlock:^(float progress) {
        if (progressBlock) {
...
+ (void)tao_imageUploadWithFunctionPath:(NSString *)functionPath
                      ...
                      progressBlock:(BJHTTPProgressBlock)progressBlock
                       successBlock:(BJServiceSuccessBlock)successBlock
                         errorBlock:(BJServiceErrorBlock)errorBlock {
    [[BlysaBJHTTPServiceEngine sharedInstance].httpEngine tao_imageUploadWithFunctionPath:functionPath params:params imageData:imageData imageName:imageName progressBlock:^(float progress) {
        if (progressBlock) {
            progressBlock(progress);
        }
    } successBlock:^(NSURLSessionDataTask *task, id responseData) {
