...
@implementation UIImageView (BlysaSVG)
- (void)svg_setImageWithURL:(nullable NSURL *)url placeholderImage:(nullable UIImage *)placeholder {
    ...
    SVGKImage *cacheObject = [TuTuosHTUserManager.manager.svgImageCache objectForKey:urlKey];
    if (cacheObject) {
        if(cacheObject.UIImage){
            [self setImage:cacheObject.UIImage];
            return;
        }
       [TuTuosHTUserManager.manager.svgImageCache removeObjectForKey:urlKey];
    }
...
//                [NSUserDefaults.standardUserDefaults setObject:svgImageData forKey:];
                 [TuTuosHTUserManager.manager.svgImageCache setObject:theSvgImage forKey:urlKey];
            }
        });
