...
@interface PXFunConfigCoreUtil()


@end

@implementation PXFunConfigCoreUtil

static PXFunConfigCoreUtil * configUtil;
...
+ (PXFunConfigCoreUtil *)share
{
    ...
    dispatch_once(&onceToken,^{
        configUtil = [[PXFunConfigCoreUtil alloc] init];
    });
    return configUtil;
}
...
-(void)saveAccount:(NSString *) mAccount password:(NSString *) password updateTime:(BOOL) updateTime
{
    ...
    for (RRDogAccountModel *am in mAccountArray) {
        if ([am.accountName isEqualToString:mAccount]) {
            am.accountPwd = password;
            if (updateTime) {
                am.lastLoginTime = [PXFunConfigCoreUtil getTimeStamp];
            }
    ...
    RRDogAccountModel *mAccountModel = [[RRDogAccountModel alloc] init];
    //赋值
    mAccountModel.lastLoginTime = [PXFunConfigCoreUtil getTimeStamp];
...
-(void)removeAccount:(NSString *) mAccount
{
    ...
    for (RRDogAccountModel *am in dataList) {
        if ([am.accountName isEqualToString:mAccount]) {
...
-(void)saveAccountModels:(NSArray<RRDogAccountModel *> *) mAccountModelArray
{
    ...
    for (RRDogAccountModel *m in mAccountModelArray) {
        //存储到NSUserDefaults
