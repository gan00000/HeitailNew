...
@interface WSKggConfigCoreUtil()


@end

@implementation WSKggConfigCoreUtil

static WSKggConfigCoreUtil * configUtil;
...
+ (WSKggConfigCoreUtil *)share
{
    ...
    dispatch_once(&onceToken,^{
        configUtil = [[WSKggConfigCoreUtil alloc] init];
    });
    return configUtil;
}
...
-(void)saveAccount:(NSString *) mAccount password:(NSString *) password updateTime:(BOOL) updateTime
{
    ...
    for (NSNiceAccountModel *am in mAccountArray) {
        if ([am.accountName isEqualToString:mAccount]) {
            am.accountPwd = password;
            if (updateTime) {
                am.lastLoginTime = [WSKggConfigCoreUtil getTimeStamp];
            }
    ...
    NSNiceAccountModel *mAccountModel = [[NSNiceAccountModel alloc] init];
    //赋值
    mAccountModel.lastLoginTime = [WSKggConfigCoreUtil getTimeStamp];
...
-(void)removeAccount:(NSString *) mAccount
{
    ...
    for (NSNiceAccountModel *am in dataList) {
        if ([am.accountName isEqualToString:mAccount]) {
...
-(void)saveAccountModels:(NSArray<NSNiceAccountModel *> *) mAccountModelArray
{
    ...
    for (NSNiceAccountModel *m in mAccountModelArray) {
        //存储到NSUserDefaults
