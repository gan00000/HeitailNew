...
#import "WSKggTZProgressView.h"
@interface WSKggTZAssetCell ()
@property (weak, nonatomic) UIImageView *imageView;       
@property (weak, nonatomic) UIImageView *selectImageView;
@property (weak, nonatomic) UIView *bottomView;
@property (weak, nonatomic) UILabel *timeLength;
@property (nonatomic, weak) UIImageView *videoImgView;
@property (nonatomic, strong) WSKggTZProgressView *progressView;
@property (nonatomic, assign) int32_t bigImageRequestID;
@end
@implementation WSKggTZAssetCell
- (void)setModel:(UUaksTZAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[NSNiceTZImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[NSNiceTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[NSNiceTZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)fetchBigImage {
    _bigImageRequestID = [[NSNiceTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (WSKggTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[WSKggTZProgressView alloc] init];
...
- (void)setModel:(TZAlbumModel *)model {
    ...
    [[NSNiceTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
