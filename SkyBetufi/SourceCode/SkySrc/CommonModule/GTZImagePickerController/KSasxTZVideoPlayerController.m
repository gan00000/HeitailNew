...
#import "FFlaliDRDeleteAlertView.h"
@interface KSasxTZVideoPlayerController () {
    AVPlayer *_player;
...
@implementation KSasxTZVideoPlayerController
...
- (void)configMoviePlayer {
    if (self.coverImg) {
        ...
        [[NSNiceTZImageManager manager] getOriginalPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info) {
            _cover = photo;
        }];
    }
    if (self.videoPath.length > 0) {
        ...
        kWeakSelf
        [[NSNiceTZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)delectButtonClick {
    [FFlaliDRDeleteAlertView showDeleteAlertViewWithComplete:^(BOOL isDelete) {
        if (isDelete) {
