...
#import "SundayDRDeleteAlertView.h"
@interface MMTodayTZVideoPlayerController () {
    AVPlayer *_player;
...
@implementation MMTodayTZVideoPlayerController
...
- (void)configMoviePlayer {
    if (self.coverImg) {
        ...
        [[PXFunTZImageManager manager] getOriginalPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info) {
            _cover = photo;
        }];
    }
    if (self.videoPath.length > 0) {
        ...
        kWeakSelf
        [[PXFunTZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)delectButtonClick {
    [SundayDRDeleteAlertView showDeleteAlertViewWithComplete:^(BOOL isDelete) {
        if (isDelete) {
