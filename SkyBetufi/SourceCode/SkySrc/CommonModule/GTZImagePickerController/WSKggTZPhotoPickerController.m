#import "WSKggTZPhotoPickerController.h"
#import "BByasTZImagePickerController.h"
#import "WSKggTZPhotoPreviewController.h"
#import "BByasTZAssetCell.h"
#import "UUaksTZAssetModel.h"
#import "UIView+YYPackageGlodBuleLayout.h"
#import "WSKggTZImageManager.h"
#import "UUaksTZVideoPlayerController.h"
#import "NSNiceTZGifPhotoPreviewController.h"
#import "FFlaliTZLocationManager.h"
@interface WSKggTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation WSKggTZPhotoPickerController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    BByasTZImagePickerController *tzImagePickerVc = (BByasTZImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            [[WSKggTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
                _model = model;
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[WSKggTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<UUaksTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[BByasTZAssetCell class] forCellWithReuseIdentifier:@"BByasTZAssetCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    [WSKggTZImageManager manager].columnNumber = [WSKggTZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    WSKggTZPhotoPreviewController *photoPreviewVc = [[WSKggTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    [WSKggTZImageManager manager].shouldFixOrientation = YES;
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[WSKggTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BByasTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BByasTZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        BByasTZImagePickerController *tzImagePickerVc = (BByasTZImagePickerController *)weakSelf.navigationController;
        if (isSelected) {
            weakCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (UUaksTZAssetModel *model_item in selectedModels) {
                if ([[[WSKggTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[WSKggTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            BByasTZImagePickerController *imagePickerVc = (BByasTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            UUaksTZVideoPlayerController *videoPlayerVc = [[UUaksTZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            NSNiceTZGifPhotoPreviewController *gifPreviewVc = [[NSNiceTZGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    __weak typeof(self) weakSelf = self;
    [[FFlaliTZLocationManager manager] startLocationWithSuccessBlock:^(CLLocation *location, CLLocation *oldLocation) {
...
- (void)pushPhotoPrevireViewController:(WSKggTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    BByasTZImagePickerController *imagePickerVc = (BByasTZImagePickerController *)self.navigationController;
    [[WSKggTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (UUaksTZAssetModel *model in _models) {
        ...
        if ([[WSKggTZImageManager manager] isAssetsArray:selectedAssets containAsset:model.asset]) {
            model.isSelected = YES;
        }
    }
}
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[WSKggTZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
...
- (void)resetCachedAssets {
    [[WSKggTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[WSKggTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[WSKggTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
