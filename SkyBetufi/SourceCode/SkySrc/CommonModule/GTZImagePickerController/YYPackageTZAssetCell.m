...
#import "NDeskTZProgressView.h"
@interface YYPackageTZAssetCell ()
@property (weak, nonatomic) UIImageView *imageView;       
@property (weak, nonatomic) UIImageView *selectImageView;
@property (weak, nonatomic) UIView *bottomView;
@property (weak, nonatomic) UILabel *timeLength;
@property (nonatomic, weak) UIImageView *videoImgView;
@property (nonatomic, strong) NDeskTZProgressView *progressView;
@property (nonatomic, assign) int32_t bigImageRequestID;
@end
@implementation YYPackageTZAssetCell
- (void)setModel:(GGCatTZAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[PXFunTZImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[PXFunTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[PXFunTZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)fetchBigImage {
    _bigImageRequestID = [[PXFunTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (NDeskTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[NDeskTZProgressView alloc] init];
...
- (void)setModel:(TZAlbumModel *)model {
    ...
    [[PXFunTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
