...
@interface TuTuosTZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
@end
@implementation TuTuosTZImagePickerController
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[TuTuosTZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[TuTuosTZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[NSNiceTZImageManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    BlysaTZPhotoPreviewController *previewVc = [[BlysaTZPhotoPreviewController alloc] init];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        KSasxTZPhotoPickerController *photoPickerVc = [[KSasxTZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[NSNiceTZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(TZAlbumModel *model) {
...
- (void)setPickerDelegate:(id<TZImagePickerControllerDelegate>)pickerDelegate {
    ...
    [NSNiceTZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [NSNiceTZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}
- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [NSNiceTZImageManager manager].photoWidth = photoWidth;
}
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    _selectedAssets = selectedAssets;
    _selectedModels = [NSMutableArray array];
    for (id asset in selectedAssets) {
        UUaksTZAssetModel *model = [UUaksTZAssetModel modelWithAsset:asset type:[[NSNiceTZImageManager manager] getAssetType:asset]];
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [NSNiceTZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)configTableView {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        TuTuosTZImagePickerController *imagePickerVc = (TuTuosTZImagePickerController *)self.navigationController;
        [[NSNiceTZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
