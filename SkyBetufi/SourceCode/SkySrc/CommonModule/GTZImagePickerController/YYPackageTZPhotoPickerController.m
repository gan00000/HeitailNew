#import "YYPackageTZPhotoPickerController.h"
#import "GGCatTZImagePickerController.h"
#import "MMTodayTZPhotoPreviewController.h"
#import "YYPackageTZAssetCell.h"
#import "GGCatTZAssetModel.h"
#import "UIView+YYPackageGlodBuleLayout.h"
#import "PXFunTZImageManager.h"
#import "MMTodayTZVideoPlayerController.h"
#import "NDeskTZGifPhotoPreviewController.h"
#import "YYPackageTZLocationManager.h"
@interface YYPackageTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@implementation YYPackageTZPhotoPickerController
...
- (void)fetchAssetModels {
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            [[PXFunTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
                _model = model;
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[PXFunTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<GGCatTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[YYPackageTZAssetCell class] forCellWithReuseIdentifier:@"YYPackageTZAssetCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    [PXFunTZImageManager manager].columnNumber = [PXFunTZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    MMTodayTZPhotoPreviewController *photoPreviewVc = [[MMTodayTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        GGCatTZAssetModel *model = tzImagePickerVc.selectedModels[i];
        [[PXFunTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    YYPackageTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"YYPackageTZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        GGCatTZImagePickerController *tzImagePickerVc = (GGCatTZImagePickerController *)weakSelf.navigationController;
        if (isSelected) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GGCatTZAssetModel *model = _models[index];
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            GGCatTZImagePickerController *imagePickerVc = (GGCatTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            MMTodayTZVideoPlayerController *videoPlayerVc = [[MMTodayTZVideoPlayerController alloc] init];
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            NDeskTZGifPhotoPreviewController *gifPreviewVc = [[NDeskTZGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    __weak typeof(self) weakSelf = self;
    [[YYPackageTZLocationManager manager] startLocationWithSuccessBlock:^(CLLocation *location, CLLocation *oldLocation) {
...
- (void)pushPhotoPrevireViewController:(MMTodayTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    GGCatTZImagePickerController *imagePickerVc = (GGCatTZImagePickerController *)self.navigationController;
    [[PXFunTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (GGCatTZAssetModel *model in _models) {
        ...
        GGCatTZImagePickerController *tzImagePickerVc = (GGCatTZImagePickerController *)self.navigationController;
        for (GGCatTZAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[PXFunTZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
...
- (void)resetCachedAssets {
    [[PXFunTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[PXFunTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[PXFunTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            GGCatTZAssetModel *model = _models[indexPath.item];
