...
#import "WSKggTZPhotoPreviewCell.h"
...
#import "NSNiceTZImageCropManager.h"
@interface WSKggTZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@end
@implementation WSKggTZPhotoPreviewController
...
- (void)viewDidLoad {
    ...
    BByasTZImagePickerController *_tzImagePickerVc = (BByasTZImagePickerController *)weakSelf.navigationController;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [WSKggTZImageManager manager].shouldFixOrientation = NO;
}
...
- (void)configBottomToolBar {
    ...
    BByasTZImagePickerController *_tzImagePickerVc = (BByasTZImagePickerController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[WSKggTZPhotoPreviewCell class] forCellWithReuseIdentifier:@"WSKggTZPhotoPreviewCell"];
...
- (void)configCropView {
    ...
    if (!_tzImagePickerVc.showSelectBtn && _tzImagePickerVc.allowCrop) {
        ...
        [NSNiceTZImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)select:(UIButton *)selectButton {
    BByasTZImagePickerController *_tzImagePickerVc = (BByasTZImagePickerController *)self.navigationController;
    UUaksTZAssetModel *model = _models[_currentIndex];
        ...
        for (UUaksTZAssetModel *model_item in selectedModels) {
            if ([[[WSKggTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[WSKggTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    UUaksTZAssetModel *model = selectedModelsTmp[i];
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { 
        ...
        WSKggTZPhotoPreviewCell *cell = (WSKggTZPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [NSNiceTZImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [NSNiceTZImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    BByasTZImagePickerController *_tzImagePickerVc = (BByasTZImagePickerController *)self.navigationController;
    UUaksTZAssetModel *model = _models[indexPath.row];
    ...
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == TZAssetModelMediaTypeVideo) {
        ...
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"WSKggTZPhotoPreviewCell" forIndexPath:indexPath];
        WSKggTZPhotoPreviewCell *photoPreviewCell = (WSKggTZPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[WSKggTZPhotoPreviewCell class]]) {
        [(WSKggTZPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    if (![[WSKggTZImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)showPhotoBytes {
    [[WSKggTZImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
