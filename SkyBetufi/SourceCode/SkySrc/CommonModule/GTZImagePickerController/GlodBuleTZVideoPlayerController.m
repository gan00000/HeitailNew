...
@interface GlodBuleTZVideoPlayerController () {
    AVPlayer *_player;
...
@implementation GlodBuleTZVideoPlayerController
...
- (void)configMoviePlayer {
    if (self.coverImg) {
        ...
        [[GlodBuleTZImageManager manager] getOriginalPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info) {
            _cover = photo;
        }];
    }
    if (self.videoPath.length > 0) {
        ...
        kWeakSelf
        [[GlodBuleTZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)delectButtonClick {
    [GlodBuleDRDeleteAlertView showDeleteAlertViewWithComplete:^(BOOL isDelete) {
        if (isDelete) {
