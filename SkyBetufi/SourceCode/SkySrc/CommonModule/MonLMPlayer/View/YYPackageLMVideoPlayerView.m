...
@interface YYPackageLMVideoPlayerView ()<UIGestureRecognizerDelegate>
...
// 控制，互动层
@property (nonatomic, strong) MMTodayLMPlayerControlView *playerControlView;
/** 播放器的参数模型 */
...
@implementation YYPackageLMVideoPlayerView
...
// !!!: 创建对象
+ (instancetype)videoPlayerViewWithSuperView:(UIView *)superview delegate:(id<LMVideoPlayerViewDelagate>)delegate playerStatusModel:(CCCaseLMPlayerStatusModel *)playerStatusModel {
    
    YYPackageLMVideoPlayerView *instance = [[YYPackageLMVideoPlayerView alloc] init];
...
#pragma mark - getter
- (MMTodayLMPlayerControlView *)playerControlView {
    if (!_playerControlView) {
        _playerControlView = [MMTodayLMPlayerControlView playerControlViewWithStatusModel:self.playerStatusModel];
    }
    return _playerControlView;
}

- (PXFunLMCoverControlView *)coverControlView {//封面包括  返回  分享  封面
    if (!_coverControlView) {
        _coverControlView = [[PXFunLMCoverControlView alloc] init];
    }
    return _coverControlView;
}

- (PXFunLMLoadingView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[PXFunLMLoadingView alloc] init];
    }
    return _loadingView;
}
