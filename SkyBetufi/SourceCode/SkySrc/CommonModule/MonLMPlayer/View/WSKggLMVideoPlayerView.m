...
@interface WSKggLMVideoPlayerView ()<UIGestureRecognizerDelegate>
...
// 控制，互动层
@property (nonatomic, strong) WSKggLMPlayerControlView *playerControlView;
/** 播放器的参数模型 */
...
@implementation WSKggLMVideoPlayerView
...
// !!!: 创建对象
+ (instancetype)videoPlayerViewWithSuperView:(UIView *)superview delegate:(id<LMVideoPlayerViewDelagate>)delegate playerStatusModel:(MMoogLMPlayerStatusModel *)playerStatusModel {
    
    WSKggLMVideoPlayerView *instance = [[WSKggLMVideoPlayerView alloc] init];
...
#pragma mark - getter
- (WSKggLMPlayerControlView *)playerControlView {
    if (!_playerControlView) {
        _playerControlView = [WSKggLMPlayerControlView playerControlViewWithStatusModel:self.playerStatusModel];
    }
    return _playerControlView;
}

- (BByasLMCoverControlView *)coverControlView {//封面包括  返回  分享  封面
    if (!_coverControlView) {
        _coverControlView = [[BByasLMCoverControlView alloc] init];
    }
    return _coverControlView;
}

- (TuTuosLMLoadingView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[TuTuosLMLoadingView alloc] init];
    }
    return _loadingView;
}
