...
@interface GlodBuleLMVideoPlayerView ()<UIGestureRecognizerDelegate>
@property (nonatomic, weak) id<LMVideoPlayerViewDelagate> delegate;
@property (nonatomic, weak) UIView *videoView;//视频所有view的父view,外部传入

// AVPlayerLayer视图容器层
@property (nonatomic, weak) UIView *playerLayerView;
// 控制，互动层
@property (nonatomic, strong) GlodBuleLMPlayerControlView *playerControlView;
/** 播放器的参数模型 */
...
@implementation GlodBuleLMVideoPlayerView
...
// !!!: 创建对象
+ (instancetype)videoPlayerViewWithSuperView:(UIView *)superview delegate:(id<LMVideoPlayerViewDelagate>)delegate playerStatusModel:(GlodBuleLMPlayerStatusModel *)playerStatusModel {
    
    GlodBuleLMVideoPlayerView *instance = [[GlodBuleLMVideoPlayerView alloc] init];
...
- (void)addSubviewWithCons {
    ...
    self.coverControlView.hidden = YES;
...
#pragma mark - getter
- (GlodBuleLMPlayerControlView *)playerControlView {
    if (!_playerControlView) {
        _playerControlView = [GlodBuleLMPlayerControlView playerControlViewWithStatusModel:self.playerStatusModel];
    }
    return _playerControlView;
}

- (GlodBuleLMCoverControlView *)coverControlView {//封面包括  返回  分享  封面
    if (!_coverControlView) {
        _coverControlView = [[GlodBuleLMCoverControlView alloc] init];
    }
    return _coverControlView;
}

- (GlodBuleLMLoadingView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[GlodBuleLMLoadingView alloc] init];
    }
    return _loadingView;
}
