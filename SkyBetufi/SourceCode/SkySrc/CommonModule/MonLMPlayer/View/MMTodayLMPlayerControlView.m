...
@interface MMTodayLMPlayerControlView ()
/** 加载loading */
@property (nonatomic, strong) NDeskMMMaterialDesignSpinner *activity;
/** 快进快退View */
...
@implementation MMTodayLMPlayerControlView

+ (instancetype)playerControlViewWithStatusModel:(CCCaseLMPlayerStatusModel *)playerStatusModel {
    MMTodayLMPlayerControlView *instance = [[self alloc] init];
...
- (CCCaseLMPortraitControlView *)portraitControlView {
    if (!_portraitControlView) {
        _portraitControlView = [[CCCaseLMPortraitControlView alloc] init];
    }
    return _portraitControlView;
}

- (NDeskLMLandScapeControlView *)landScapeControlView {
    if (!_landScapeControlView) {
        _landScapeControlView = [[NDeskLMLandScapeControlView alloc] init];
    }
    return _landScapeControlView;
}

- (NDeskMMMaterialDesignSpinner *)activity {
    if (!_activity) {
        _activity = [[NDeskMMMaterialDesignSpinner alloc] init];
