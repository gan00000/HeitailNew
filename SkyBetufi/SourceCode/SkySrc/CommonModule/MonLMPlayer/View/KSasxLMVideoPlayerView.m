...
@interface KSasxLMVideoPlayerView ()<UIGestureRecognizerDelegate>
...
// 控制，互动层
@property (nonatomic, strong) KSasxLMPlayerControlView *playerControlView;
/** 播放器的参数模型 */
...
@implementation KSasxLMVideoPlayerView
...
+ (instancetype)videoPlayerViewWithSuperView:(UIView *)superview delegate:(id<LMVideoPlayerViewDelagate>)delegate playerStatusModel:(MMoogLMPlayerStatusModel *)playerStatusModel {
    ...
    KSasxLMVideoPlayerView *instance = [[KSasxLMVideoPlayerView alloc] init];
...
#pragma mark - getter
- (KSasxLMPlayerControlView *)playerControlView {
    if (!_playerControlView) {
        _playerControlView = [KSasxLMPlayerControlView playerControlViewWithStatusModel:self.playerStatusModel];
    }
    return _playerControlView;
}

- (KSasxLMCoverControlView *)coverControlView {//封面包括  返回  分享  封面
    if (!_coverControlView) {
        _coverControlView = [[KSasxLMCoverControlView alloc] init];
    }
    return _coverControlView;
}

- (NSNiceLMLoadingView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[NSNiceLMLoadingView alloc] init];
    }
    return _loadingView;
}
