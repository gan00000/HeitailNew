...
#import "MMoogYSPlayerControl.h"
#import <MediaPlayer/MediaPlayer.h>
#import "BByasYSNetworkInfoTool.h"
#import "WSKggYSBatteryTool.h"
#import "FFlaliYSCarrierTool.h"
#import "BlysaYSScreenBrightnessTool.h"
...
@interface MMoogYSPlayerControl () <UIGestureRecognizerDelegate>
...
@property (strong, nonatomic) WSKggYSBatteryTool *batteryTool;
...
@implementation MMoogYSPlayerControl
...
//    self.statusBar.hidden = fullScreen ? NO : YES;
    if (fullScreen) {//全屏
        ...
        [self addPanGesture];
        if (!self.portraitFullScreen && [UUaksBJUtility isIPhoneXSeries]) {
//            self.playBtnLeadingConstraint.constant = 44;
    ...
    {//非全屏
        ...
        if ([UUaksBJUtility isIPhoneXSeries]) {
...
- (void)loadCarrier {
    ...
    [FFlaliYSCarrierTool loadCarrier:^(NSString * _Nonnull carrierName) {
...
- (void)loadNetworkInfo {
    __weak typeof(self) weakSelf = self;
    [BByasYSNetworkInfoTool loadNetworkInfo:^(NSString * _Nonnull info) {
...
- (void)loadBatteryInfo {
    __weak typeof(self) weakSelf = self;
    self.batteryTool = [WSKggYSBatteryTool batteryToolWithStatus:^(NSString * _Nonnull status, UIColor *color) {
...
- (void)initScreenBrightness {
    ...
    self.volumeProgressView.progress = [BlysaYSScreenBrightnessTool getBrightness];
}
...
- (void)changeBrightness:(CGFloat)deltaY {
    CGFloat brightness = [BlysaYSScreenBrightnessTool getBrightness];
    brightness -= deltaY;
    [BlysaYSScreenBrightnessTool changeBrightness:brightness];
