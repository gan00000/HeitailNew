...
#import "NDeskYSPlayerControl.h"
#import <MediaPlayer/MediaPlayer.h>
#import "PXFunYSNetworkInfoTool.h"
#import "GGCatYSBatteryTool.h"
#import "NDeskYSCarrierTool.h"
...
@interface NDeskYSPlayerControl () <UIGestureRecognizerDelegate>
...
@property (strong, nonatomic) GGCatYSBatteryTool *batteryTool;
@property (strong, nonatomic) KMonkeyYSVolumeTool *volumeTool;
...
@implementation NDeskYSPlayerControl
...
//    self.statusBar.hidden = fullScreen ? NO : YES;
    if (fullScreen) {//全屏
        ...
        [self addPanGesture];
        if (!self.portraitFullScreen && [KMonkeyBJUtility isIPhoneXSeries]) {
//            self.playBtnLeadingConstraint.constant = 44;
    ...
    {//非全屏
        ...
        if ([KMonkeyBJUtility isIPhoneXSeries]) {
...
- (void)addMPVolumeView {
    self.volumeTool = [[KMonkeyYSVolumeTool alloc] init];
...
- (void)loadCarrier {
    ...
    [NDeskYSCarrierTool loadCarrier:^(NSString * _Nonnull carrierName) {
...
- (void)loadNetworkInfo {
    __weak typeof(self) weakSelf = self;
    [PXFunYSNetworkInfoTool loadNetworkInfo:^(NSString * _Nonnull info) {
...
- (void)loadBatteryInfo {
    __weak typeof(self) weakSelf = self;
    self.batteryTool = [GGCatYSBatteryTool batteryToolWithStatus:^(NSString * _Nonnull status, UIColor *color) {
...
- (void)initScreenBrightness {
    ...
    self.volumeProgressView.progress = [NDeskYSScreenBrightnessTool getBrightness];
}
...
- (void)changeBrightness:(CGFloat)deltaY {
    CGFloat brightness = [NDeskYSScreenBrightnessTool getBrightness];
    brightness -= deltaY;
    [NDeskYSScreenBrightnessTool changeBrightness:brightness];
