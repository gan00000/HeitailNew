...
#import "WSKggLMVideoPlayer.h"
#import "UUaksLMPlayerManager.h"
#import "KSasxLMVideoPlayerView.h"
#import "MMoogLMPlayerStatusModel.h"
#import "MMoogLMBrightnessView.h"

@interface WSKggLMVideoPlayer ()<LMPlayerManagerDelegate, LMPlayerControlViewDelagate, LMPortraitControlViewDelegate, LMLandScapeControlViewDelegate, LMVideoPlayerViewDelagate, LMLoadingViewDelegate, LMCoverControlViewDelegate>

// 代理
@property (nonatomic, weak) id<LMVideoPlayerDelegate> delegate;
// 最底层的父视图
@property (nonatomic, strong) KSasxLMVideoPlayerView *videoPlayerView;
// AVPlayer 管理
@property (nonatomic, strong) UUaksLMPlayerManager *playerMgr;
// 播放数据模型
@property (nonatomic, strong) BlysaLMPlayerModel *playerModel;
/** 播放器的参数模型 */
@property (nonatomic, strong) MMoogLMPlayerStatusModel *playerStatusModel;
...
@implementation WSKggLMVideoPlayer
#pragma mark - getter
...
+ (instancetype)videoPlayerWithView:(UIView *)view delegate:(id<LMVideoPlayerDelegate>)delegate playerModel:(BlysaLMPlayerModel *)playerModel {
    
    if (view == nil) {
        return nil;
    }
    
    WSKggLMVideoPlayer *instance = [[WSKggLMVideoPlayer alloc] init];
    ...
    // 创建状态模型
    instance.playerStatusModel = [[MMoogLMPlayerStatusModel alloc] init];
    [instance.playerStatusModel playerResetStatusModel];
    
    // !!!: 最底层视图创建
    instance.videoPlayerView = [KSasxLMVideoPlayerView videoPlayerViewWithSuperView:view delegate:instance playerStatusModel:instance.playerStatusModel];
    ...
    // !!!: 创建AVPlayer管理
    instance.playerMgr = [UUaksLMPlayerManager playerManagerWithDelegate:instance playerStatusModel:instance.playerStatusModel];
...
//- (void)setPlayerModel:(BlysaLMPlayerModel *)playerModel {
 ...
 *  在当前页面，设置新的视频时候调用此方法
 */
- (void)resetToPlayNewVideo:(BlysaLMPlayerModel *)playerModel {
    [self resetPlayer];
//    self.playerModel = playerModel;
    [self configLMPlayer];
}

- (void)playNewVideo:(BlysaLMPlayerModel *)playerModel {
    [self resetPlayer];
    self.playerModel = playerModel;
...
- (void)didBuffer:(UUaksLMPlayerManager *)playerMgr { 
...
//    }
    ...
    BlysaLMPlayerModel *newModel = [[BlysaLMPlayerModel alloc] init];
