...
#import "YYPackageLMPlayerManager.h"
#import "YYPackageLMVideoPlayerView.h"
#import "CCCaseLMPlayerStatusModel.h"
#import "NDeskLMBrightnessView.h"

@interface GGCatLMVideoPlayer ()<LMPlayerManagerDelegate, LMPlayerControlViewDelagate, LMPortraitControlViewDelegate, LMLandScapeControlViewDelegate, LMVideoPlayerViewDelagate, LMLoadingViewDelegate, LMCoverControlViewDelegate>

// 代理
@property (nonatomic, weak) id<LMVideoPlayerDelegate> delegate;
// 最底层的父视图
@property (nonatomic, strong) YYPackageLMVideoPlayerView *videoPlayerView;
// AVPlayer 管理
@property (nonatomic, strong) YYPackageLMPlayerManager *playerMgr;
// 播放数据模型
@property (nonatomic, strong) MMTodayLMPlayerModel *playerModel;
/** 播放器的参数模型 */
@property (nonatomic, strong) CCCaseLMPlayerStatusModel *playerStatusModel;
...
@implementation GGCatLMVideoPlayer
#pragma mark - getter
...
+ (instancetype)videoPlayerWithView:(UIView *)view delegate:(id<LMVideoPlayerDelegate>)delegate playerModel:(MMTodayLMPlayerModel *)playerModel {
    
    if (view == nil) {
        return nil;
    }
    
    GGCatLMVideoPlayer *instance = [[GGCatLMVideoPlayer alloc] init];
    ...
    // 创建状态模型
    instance.playerStatusModel = [[CCCaseLMPlayerStatusModel alloc] init];
    [instance.playerStatusModel playerResetStatusModel];
    
    // !!!: 最底层视图创建
    instance.videoPlayerView = [YYPackageLMVideoPlayerView videoPlayerViewWithSuperView:view delegate:instance playerStatusModel:instance.playerStatusModel];
    ...
    // !!!: 创建AVPlayer管理
    instance.playerMgr = [YYPackageLMPlayerManager playerManagerWithDelegate:instance playerStatusModel:instance.playerStatusModel];
...
//- (void)setPlayerModel:(MMTodayLMPlayerModel *)playerModel {
 ...
 *  在当前页面，设置新的视频时候调用此方法
 */
- (void)resetToPlayNewVideo:(MMTodayLMPlayerModel *)playerModel {
    [self resetPlayer];
//    self.playerModel = playerModel;
    [self configLMPlayer];
}

- (void)playNewVideo:(MMTodayLMPlayerModel *)playerModel {
    [self resetPlayer];
    self.playerModel = playerModel;
...
- (void)didBuffer:(YYPackageLMPlayerManager *)playerMgr { 
...
//    }
    ...
    MMTodayLMPlayerModel *newModel = [[MMTodayLMPlayerModel alloc] init];
...
- (void)refreshPlayBtnClick:(BOOL)isSelected {
    // 延迟隐藏控制层
    [self.videoPlayerView.playerControlView autoFadeOutControlView];
...
- (void)panHorizontalBeginMoved {
   ...
   if (totalMovieDuration <= 0) {//直播的情況下
       return;
   }
    // 给sumTime初值
