...
@implementation MMTodayHTCourtHotShootView
...
- (void)setDataModel:(NSArray<HourseHotShootPointModel *> *) model isLeft:(BOOL) isLeft width:(CGFloat)width height:(CGFloat)height
{
    ...
    if (isLeft) {
        ...
        for (HourseHotShootPointModel *m in model) {
...
- (void)setDataModel:(NSArray<HourseHotShootPointModel *> *) model isLeft:(BOOL) isLeft width:(CGFloat)width height:(CGFloat)height
{
    NSMutableArray<HourseHotShootPointModel *> *hitDataModel = [[NSMutableArray alloc] init];
    NSMutableArray<HourseHotShootPointModel *> *notHitDataModel = [[NSMutableArray alloc] init];
    ...
    for (HourseHotShootPointModel *pm in model) {//是否命中 1-是 0-否
        ...
        if (xValue >= 0 && yValue >= 0) {
            ...
            NSLog(@"HourseHotShootPointModel xValue小于0->%f , %f",xValue,yValue);
        }
