...
- (void)showEmptyViewWithImage:(UIImage *)image title:(NSString *)title tapBlock:(dispatch_block_t)tapBlock edge:(UIEdgeInsets)edge {
    [self hideEmptyView];
    RRDogXJEmptyView *emptyView = [[NSBundle mainBundle] loadNibNamed:@"RRDogXJEmptyView" owner:nil options:nil].firstObject;
    if (title.length > 0) {
