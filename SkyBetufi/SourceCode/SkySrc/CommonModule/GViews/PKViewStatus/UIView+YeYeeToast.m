...
#import "Masonry.h"
@implementation UIView (YeYeeToast)
- (void)showToast:(NSString *)toast duration:(CGFloat)duration {
    __block CfipyXJToastView *toastView = [[NSBundle mainBundle] loadNibNamed:@"CfipyXJToastView" owner:nil options:nil].firstObject;
...
- (void)showToast:(NSString *)toast icon:(UIImage *)icon duration:(CGFloat)duration {
    __block TuTuosXJToastImageView *toastView = [[NSBundle mainBundle] loadNibNamed:@"TuTuosXJToastImageView" owner:nil options:nil].firstObject;
    toastView.toastLabel.text = toast;
