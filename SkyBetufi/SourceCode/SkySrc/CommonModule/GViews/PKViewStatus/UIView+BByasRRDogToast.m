...
#import "Masonry.h"
@implementation UIView (RRDogToast)
- (void)showToast:(NSString *)toast duration:(CGFloat)duration {
    __block UUaksXJToastView *toastView = [[NSBundle mainBundle] loadNibNamed:@"UUaksXJToastView" owner:nil options:nil].firstObject;
...
- (void)showToast:(NSString *)toast icon:(UIImage *)icon duration:(CGFloat)duration {
    __block BlysaXJToastImageView *toastView = [[NSBundle mainBundle] loadNibNamed:@"BlysaXJToastImageView" owner:nil options:nil].firstObject;
    toastView.toastLabel.text = toast;
