...
#import "NSNiceXJEmptyView.h"
#import "Masonry.h"
#import "AFNetworkReachabilityManager.h"
static void * kEmptyViewKey;
@implementation UIView (PXFunEmptyView)
- (void)setEmptyView:(NSNiceXJEmptyView *)emptyView {
    objc_setAssociatedObject(self, &kEmptyViewKey, emptyView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSNiceXJEmptyView *)emptyView {
...
- (void)showEmptyViewWithImage:(UIImage *)image title:(NSString *)title tapBlock:(dispatch_block_t)tapBlock edge:(UIEdgeInsets)edge {
    [self hideEmptyView];
    NSNiceXJEmptyView *emptyView = [[NSBundle mainBundle] loadNibNamed:@"NSNiceXJEmptyView" owner:nil options:nil].firstObject;
    if (title.length > 0) {
