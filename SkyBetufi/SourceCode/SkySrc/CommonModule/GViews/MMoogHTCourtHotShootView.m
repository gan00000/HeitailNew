...
@interface MMoogHTCourtHotShootView()
...
@implementation MMoogHTCourtHotShootView
...
- (void)setDataModel:(NSArray<BByasHotShootPointModel *> *) model isLeft:(BOOL) isLeft width:(CGFloat)width height:(CGFloat)height
{
    ...
    if (isLeft) {
        ...
        for (BByasHotShootPointModel *m in model) {
...
- (void)setDataModel:(NSArray<BByasHotShootPointModel *> *) model isLeft:(BOOL) isLeft width:(CGFloat)width height:(CGFloat)height
{
    NSMutableArray<BByasHotShootPointModel *> *hitDataModel = [[NSMutableArray alloc] init];
    NSMutableArray<BByasHotShootPointModel *> *notHitDataModel = [[NSMutableArray alloc] init];
    ...
    for (BByasHotShootPointModel *pm in model) {//是否命中 1-是 0-否
        ...
        if (xValue >= 0 && yValue >= 0) {
            ...
            NSLog(@"BByasHotShootPointModel xValue小于0->%f , %f",xValue,yValue);
        }
