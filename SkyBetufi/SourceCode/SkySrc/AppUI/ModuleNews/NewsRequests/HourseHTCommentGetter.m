...
@interface HourseHTCommentGetter ()
...
@end
@implementation HourseHTCommentGetter
...
- (void)loadHotCommentsWithCompleteBlock:(dispatch_block_t)block {
    [GGCatHTNewsAdditionRequest taorequestHotCommentWithOffset:self.hotOffset newsId:self.post_id successBlock:^(NSArray<HourseHTCommentModel *> * _Nonnull commentList, NSInteger pages) {
        if (self.hotOffset == 0) {
        ...
        [[PXFunPPXXBJViewControllerCenter currentViewController].view showToast:@"獲取「熱門評論」失敗"];
        if (block) {
            block();
        }
    }];
...
- (void)loadNormalCommentsWithCompleteBlock:(dispatch_block_t)block {
    [GGCatHTNewsAdditionRequest taorequestNormalCommentWithOffset:self.normalOffset newsId:self.post_id successBlock:^(NSArray<HourseHTCommentModel *> * _Nonnull commentList, NSInteger pages) {
        ...
        [[PXFunPPXXBJViewControllerCenter currentViewController].view showToast:@"獲取「全部評論」失敗"];
        if (block) {
            block();
        }
    }];
}
- (void)countHeightForComments:(NSArray<HourseHTCommentModel *> *)comments {
    for (HourseHTCommentModel *commentModel in comments) {
