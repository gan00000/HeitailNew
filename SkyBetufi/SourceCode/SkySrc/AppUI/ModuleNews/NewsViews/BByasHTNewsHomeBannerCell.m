...
@interface BByasHTNewsHomeBannerCell () <UIScrollViewDelegate>
...
@end
@implementation BByasHTNewsHomeBannerCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)taosetupWithNewsModels:(NSArray *)bannerList {
    ...
    if (self.bannerList.count > 0) {
        for (int i = 0; i < self.bannerList.count; i++) {
            ...
            UUaksHTNewsModel *banner = self.bannerList[i];
        ...
        if (self.bannerList.count > 1) {
            ...
            [self.scrollView addSubview:imageView];
            UUaksHTNewsModel *lastBanner = [self.bannerList lastObject];
            ...
            imageView.tag = 0;
            [self.scrollView addSubview:imageView];
            UUaksHTNewsModel *firstBanner = [self.bannerList firstObject];
...
- (void)onBannerImageTapped:(UITapGestureRecognizer *)tap {
    ...
    UUaksHTNewsModel *banner = self.bannerList[tapImageView.tag];
...
- (void)p_setupTitle {
    ...
    UUaksHTNewsModel *currentModel = self.bannerList[index];
