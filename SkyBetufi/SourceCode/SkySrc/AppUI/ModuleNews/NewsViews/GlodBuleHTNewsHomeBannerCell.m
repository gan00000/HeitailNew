...
@interface GlodBuleHTNewsHomeBannerCell () <UIScrollViewDelegate>
...
@end
@implementation GlodBuleHTNewsHomeBannerCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)taosetupWithNewsModels:(NSArray *)bannerList {
    ...
    if (self.bannerList.count > 0) {
        for (int i = 0; i < self.bannerList.count; i++) {
            ...
            GlodBuleHTNewsModel *banner = self.bannerList[i];
        ...
        if (self.bannerList.count > 1) {
            ...
            [self.scrollView addSubview:imageView];
            GlodBuleHTNewsModel *lastBanner = [self.bannerList lastObject];
            ...
            imageView.tag = 0;
            [self.scrollView addSubview:imageView];
            GlodBuleHTNewsModel *firstBanner = [self.bannerList firstObject];
...
- (void)onBannerImageTapped:(UITapGestureRecognizer *)tap {
    ...
    GlodBuleHTNewsModel *banner = self.bannerList[tapImageView.tag];
...
- (void)p_setupTitle {
    ...
    GlodBuleHTNewsModel *currentModel = self.bannerList[index];
