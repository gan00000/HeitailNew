...
@property (nonatomic, weak) HourseHTCommentModel *commentModel;
@end
@implementation NDeskHTNewsCommentCell
- (void)awakeFromNib {
    [super awakeFromNib];
    ...
    [self.replyTableView registerNib:[UINib nibWithNibName:NSStringFromClass([RRDogHTCommentExpendCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([RRDogHTCommentExpendCell class])];
...
//    self.avatarImageView.image = commentModel.userModel.avatar;
    ...
    self.timelabel.text = [GGCatBJDateFormatUtility dateToShowFromDate:commentModel.date];
...
- (IBAction)onLikeAction:(UIButton *)sender {
    ...
    UIView *view = [PXFunPPXXBJViewControllerCenter currentViewController].view;
    [HourseHTUserRequest taolikePostWithPostId:self.commentModel.post_id comment_id:self.commentModel.comment_id like:!sender.selected successBlock:^{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        ...
        indexPath.row < 3) {
        NDeskHTNewsCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self class])];
    ...
    kWeakSelf
    RRDogHTCommentExpendCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([RRDogHTCommentExpendCell class])];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        (self.commentModel.expend && indexPath.row < self.commentModel.reply.count) ||
        indexPath.row < 3) {
        HourseHTCommentModel *model = self.commentModel.reply[indexPath.row];
