...
#import "GlodBuleHTCommentExpendCell.h"
#import <YYText/YYText.h>
#import "UIImageView+GlodBuleHT.h"

@interface GlodBuleHTNewsCommentCell () <UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, weak) GlodBuleHTCommentModel *commentModel;
@end
@implementation GlodBuleHTNewsCommentCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)taorefreshWithCommentModel:(GlodBuleHTCommentModel *)commentModel {
...
//    self.avatarImageView.image = commentModel.userModel.avatar;
    ...
    self.timelabel.text = [GlodBuleBJDateFormatUtility dateToShowFromDate:commentModel.date];
...
- (IBAction)onLikeAction:(UIButton *)sender {
    ...
    UIView *view = [GlodBulePPXXBJViewControllerCenter currentViewController].view;
    [GlodBuleHTUserRequest taolikePostWithPostId:self.commentModel.post_id comment_id:self.commentModel.comment_id like:!sender.selected successBlock:^{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        ...
        indexPath.row < 3) {
        GlodBuleHTNewsCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self class])];
    ...
    kWeakSelf
    GlodBuleHTCommentExpendCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([GlodBuleHTCommentExpendCell class])];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        (self.commentModel.expend && indexPath.row < self.commentModel.reply.count) ||
        indexPath.row < 3) {
        GlodBuleHTCommentModel *model = self.commentModel.reply[indexPath.row];
