...
@interface KMonkeyHTNewsHomeBannerCell () <UIScrollViewDelegate>
...
@end
@implementation KMonkeyHTNewsHomeBannerCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)taosetupWithNewsModels:(NSArray *)bannerList {
    ...
    if (self.bannerList.count > 0) {
        for (int i = 0; i < self.bannerList.count; i++) {
            ...
            PXFunHTNewsModel *banner = self.bannerList[i];
        ...
        if (self.bannerList.count > 1) {
            ...
            [self.scrollView addSubview:imageView];
            PXFunHTNewsModel *lastBanner = [self.bannerList lastObject];
            ...
            imageView.tag = 0;
            [self.scrollView addSubview:imageView];
            PXFunHTNewsModel *firstBanner = [self.bannerList firstObject];
...
- (void)onBannerImageTapped:(UITapGestureRecognizer *)tap {
    ...
    PXFunHTNewsModel *banner = self.bannerList[tapImageView.tag];
...
- (void)p_setupTitle {
    ...
    PXFunHTNewsModel *currentModel = self.bannerList[index];
