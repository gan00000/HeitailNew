...
#import "FFlaliHTCommentExpendCell.h"
#import <YYText/YYText.h>
#import "UIImageView+WSKggPXFunHT.h"

@interface BByasHTNewsCommentCell () <UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, weak) NSNiceHTCommentModel *commentModel;
@end
@implementation BByasHTNewsCommentCell
- (void)awakeFromNib {
    [super awakeFromNib];
    ...
    [self.replyTableView registerNib:[UINib nibWithNibName:NSStringFromClass([FFlaliHTCommentExpendCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([FFlaliHTCommentExpendCell class])];
...
//    self.avatarImageView.image = commentModel.userModel.avatar;
    ...
    self.timelabel.text = [FFlaliBJDateFormatUtility dateToShowFromDate:commentModel.date];
...
- (IBAction)onLikeAction:(UIButton *)sender {
    ...
    UIView *view = [UUaksPPXXBJViewControllerCenter currentViewController].view;
    [TuTuosHTUserRequest taolikePostWithPostId:self.commentModel.post_id comment_id:self.commentModel.comment_id like:!sender.selected successBlock:^{
        ...
        [self taorefreshWithCommentModel:self.commentModel];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        ...
        indexPath.row < 3) {
        BByasHTNewsCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self class])];
        cell.onReplyBlock = ^(NSNiceHTCommentModel * _Nonnull commentModel) {
            if (self.onReplyBlock) {
                self.onReplyBlock(commentModel);
            }
        };
        [cell taorefreshWithCommentModel:self.commentModel.reply[indexPath.row]];
        return cell;
    }
    kWeakSelf
    FFlaliHTCommentExpendCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([FFlaliHTCommentExpendCell class])];
    cell.onExpendChangeBlock = ^(BOOL expend) {
        weakSelf.commentModel.expend = expend;
        [weakSelf taorefreshWithCommentModel:weakSelf.commentModel];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.commentModel.reply.count <= 3 ||
        (self.commentModel.expend && indexPath.row < self.commentModel.reply.count) ||
        indexPath.row < 3) {
        NSNiceHTCommentModel *model = self.commentModel.reply[indexPath.row];
