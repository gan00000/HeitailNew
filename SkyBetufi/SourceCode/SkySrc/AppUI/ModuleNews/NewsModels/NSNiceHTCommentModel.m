...
@interface NSNiceHTCommentModel ()
...
@end
@implementation NSNiceHTCommentModel
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"like": [TuTuosHTUserInfoModel class],
             @"reply": [NSNiceHTCommentModel class]
             };
}
- (void)taocountHeight:(BOOL)isReply {
    ...
    if (!isReply) {
        ...
        for (NSInteger i=0; i<self.reply.count; i++) {
            NSNiceHTCommentModel *model = self.reply[i];
            [model taocountHeight:YES];
            if (i < 3) {
...
- (TuTuosHTUserInfoModel *)userModel {
    if (!_userModel) {
        _userModel = [[TuTuosHTUserInfoModel alloc] init];
