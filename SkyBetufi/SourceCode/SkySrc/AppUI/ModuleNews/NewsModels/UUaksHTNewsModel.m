...
@interface UUaksHTNewsModel () <NSURLConnectionDelegate>
@property (nonatomic, copy) NSString *clearContent;
@end
@implementation UUaksHTNewsModel
...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"hottest_comment": [NSNiceHTCommentModel class]
             };
}
...
- (void)setContent:(NSString *)content {
    ...
    if (iframe) {
        ...
        NSLog(@"iframe = %@", iframe);
        _iframe = [[MMoogHTHtmlLoadUtil sharedInstance] iframHtmlWithContent:iframe];
        NSLog(@"iframe = %@", _iframe);
    } 
}
- (void)setDate:(NSString *)date {
    _date = date;
    _time = [FFlaliBJDateFormatUtility dateToShowFromDateString:date];
}
...
- (void)taoshare {
    kWeakSelf
    [TuTuosHTLoginAlertView taoshowShareAlertViewWithSelectBlock:^(HTLoginPlatform platform) {
...
//            [self doShareToPlatform:UMSocialPlatformType_Facebook withMessage:messageObject];
            ...
            [FBSDKShareDialog showFromViewController:[UUaksPPXXBJViewControllerCenter currentViewController]
                                          withContent:content
                                             delegate:nil];
            ...
            if (weakSelf.img_url) {
                [NSNiceBJLoadingHud showHUDInView:[UUaksPPXXBJViewControllerCenter currentViewController].view];
                [[SDWebImageManager sharedManager] loadImageWithURL:[NSURL URLWithString:weakSelf.img_url] options:0 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                    ...
                    [NSNiceBJLoadingHud hideHUDInView:[UUaksPPXXBJViewControllerCenter currentViewController].view];
                }];
            } else {
                [weakSelf doShareToPlatform:UMSocialPlatformType_Line withMessage:messageObject];
            }
...
- (TuTuosHTUserInfoModel *)userInfo {
    if (!_userInfo) {
        _userInfo = [[TuTuosHTUserInfoModel alloc] init];
