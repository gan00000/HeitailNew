...
@interface PXFunHTNewsModel () <NSURLConnectionDelegate>
...
- (void)setContent:(NSString *)content {
    ...
    if (iframe) {
        ...
        NSLog(@"iframe = %@", iframe);
        _iframe = [[MMTodayHTHtmlLoadUtil sharedInstance] iframHtmlWithContent:iframe];
        NSLog(@"iframe = %@", _iframe);
    } 
}
- (void)setDate:(NSString *)date {
    _date = date;
    _time = [GGCatBJDateFormatUtility dateToShowFromDateString:date];
}
...
- (GGCatHTUserInfoModel *)userInfo {
    if (!_userInfo) {
        _userInfo = [[GGCatHTUserInfoModel alloc] init];
