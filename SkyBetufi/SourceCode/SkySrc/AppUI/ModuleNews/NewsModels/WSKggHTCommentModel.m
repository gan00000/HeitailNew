...
@end
@implementation WSKggHTCommentModel
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"like": [FFlaliHTUserInfoModel class],
             @"reply": [WSKggHTCommentModel class]
             };
}
- (void)taocountHeight:(BOOL)isReply {
    ...
    if (!isReply) {
        ...
        for (NSInteger i=0; i<self.reply.count; i++) {
            WSKggHTCommentModel *model = self.reply[i];
...
- (FFlaliHTUserInfoModel *)userModel {
    if (!_userModel) {
        _userModel = [[FFlaliHTUserInfoModel alloc] init];
