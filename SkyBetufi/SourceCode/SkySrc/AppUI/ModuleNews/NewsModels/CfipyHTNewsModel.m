...
@interface CfipyHTNewsModel () <NSURLConnectionDelegate>
@property (nonatomic, copy) NSString *clearContent;
@end
@implementation CfipyHTNewsModel
...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"hottest_comment": [WSKggHTCommentModel class]
             };
}
...
- (void)setContent:(NSString *)content {
    ...
    if (iframe) {
        ...
        NSLog(@"iframe = %@", iframe);
        _iframe = [[TuTuosHTHtmlLoadUtil sharedInstance] iframHtmlWithContent:iframe];
        NSLog(@"iframe = %@", _iframe);
    } 
}
- (void)setDate:(NSString *)date {
    _date = date;
    _time = [BlysaBJDateFormatUtility dateToShowFromDateString:date];
}
...
- (FFlaliHTUserInfoModel *)userInfo {
    if (!_userInfo) {
        _userInfo = [[FFlaliHTUserInfoModel alloc] init];
