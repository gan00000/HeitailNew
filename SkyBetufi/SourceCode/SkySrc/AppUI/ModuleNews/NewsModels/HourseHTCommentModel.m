...
@interface HourseHTCommentModel ()
...
@end
@implementation HourseHTCommentModel
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"like": [GGCatHTUserInfoModel class],
             @"reply": [HourseHTCommentModel class]
             };
}
- (void)taocountHeight:(BOOL)isReply {
    ...
    if (!isReply) {
        ...
        for (NSInteger i=0; i<self.reply.count; i++) {
            HourseHTCommentModel *model = self.reply[i];
...
- (GGCatHTUserInfoModel *)userModel {
    if (!_userModel) {
        _userModel = [[GGCatHTUserInfoModel alloc] init];
