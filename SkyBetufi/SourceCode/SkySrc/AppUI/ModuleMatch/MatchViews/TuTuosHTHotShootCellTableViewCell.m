//
//  TuTuosHTHotShootCellTableViewCell.m
//  
//
...
#import "MMoogGamaAlertView.h"
#import "UIView+WSKggRRDogViewController.h"
#import "MMoogHTMatchSummaryRequest.h"
#import "UIImageView+WSKggPXFunHT.h"

@interface TuTuosHTHotShootCellTableViewCell ()
...
@property (nonatomic, strong)NSArray<BByasHotShootPointModel *> *hotShootPointModel_away_temp;
@property (nonatomic, strong)NSArray<BByasHotShootPointModel *> *hotShootPointModel_home_temp;
...
@implementation TuTuosHTHotShootCellTableViewCell
...
-(void) updateMatchInfoWiithSummaryModel:(NSNiceHTMatchSummaryModel *)summaryModel matchCompareModel:(WSKggHTMatchCompareModel *)matchCompareModel gameId:(NSString*)gameId
{
    ...
    for (TuTuosHTMatchDetailsModel *m in matchCompareModel.awayTeamDetails) {
    ...
    for (TuTuosHTMatchDetailsModel *m in matchCompareModel.homeTeamDetails) {
...
-(void)clickSelectQuartView{
    [MMoogGamaAlertView showActionSheetWithTitle:nil
                                    message:nil
...
{
    kWeakSelf
    [MMoogHTMatchSummaryRequest getShootPointWithGameId:self.game_id home_away:team playerId:playerId quarter:self.quartId successBlock:^(NSArray<BByasHotShootPointModel *> *model) {
...
-(NSString *)setHitRate:(NSArray<BByasHotShootPointModel *> *)model
{
    CGFloat hitCount = 0;
    NSUInteger allCount = model.count;
    if (model) {
        for (BByasHotShootPointModel *x in model) {
