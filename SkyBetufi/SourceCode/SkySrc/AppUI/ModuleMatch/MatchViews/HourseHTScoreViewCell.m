...
@property (nonatomic,strong) HourseLineChartUtil *mLineChartUtil;
@property (nonatomic) BOOL isToViewAdd;

@end
@implementation HourseHTScoreViewCell
...
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
    _mLineChartUtil = [[HourseLineChartUtil alloc]init];
...
- (void)setMatchSummaryModel:(NDeskHTMatchSummaryModel *)summaryModel liveFeedModel:(NSArray<SundayHTMatchLiveFeedModel *> *)liveFeedModel
{
    ...
    NSArray<SundayHTMatchLiveFeedModel *> *c_liveFeedModel = [liveFeedModel copy];
    ...
    int yMax = 0;
    int tempSeconds = 0;
    for (SundayHTMatchLiveFeedModel *feedMode in c_liveFeedModel) {
...
//        BJLog(@"quarter = %ld",quarter);
    ...
    SundayHTMatchLiveFeedModel *last_feedMode = c_liveFeedModel.lastObject;
