//
//  HourseHTHotShootCellTableViewCell.m
//  
//
...
@property (nonatomic, strong)NSArray<HourseHotShootPointModel *> *hotShootPointModel_away_temp;
@property (nonatomic, strong)NSArray<HourseHotShootPointModel *> *hotShootPointModel_home_temp;
...
@implementation HourseHTHotShootCellTableViewCell
...
-(void) updateMatchInfoWiithSummaryModel:(NDeskHTMatchSummaryModel *)summaryModel matchCompareModel:(PXFunHTMatchCompareModel *)matchCompareModel gameId:(NSString*)gameId
{
    ...
    for (SundayHTMatchDetailsModel *m in matchCompareModel.awayTeamDetails) {
    ...
    for (SundayHTMatchDetailsModel *m in matchCompareModel.homeTeamDetails) {
...
-(void)clickSelectQuartView{
    [RRDogGamaAlertView showActionSheetWithTitle:nil
                                    message:nil
...
{
    kWeakSelf
    [PXFunHTMatchSummaryRequest getShootPointWithGameId:self.game_id home_away:team playerId:playerId quarter:self.quartId successBlock:^(NSArray<HourseHotShootPointModel *> *model) {
...
-(NSString *)setHitRate:(NSArray<HourseHotShootPointModel *> *)model
{
    CGFloat hitCount = 0;
    NSUInteger allCount = model.count;
    if (model) {
        for (HourseHotShootPointModel *x in model) {
