...
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
    _mLineChartUtil = [[MMoogLineChartUtil alloc]init];
...
- (void)setMatchSummaryModel:(NSNiceHTMatchSummaryModel *)summaryModel liveFeedModel:(NSArray<UUaksHTMatchLiveFeedModel *> *)liveFeedModel
{
    ...
    NSArray<UUaksHTMatchLiveFeedModel *> *c_liveFeedModel = [liveFeedModel copy];
    ...
    int yMax = 0;
    int tempSeconds = 0;
    for (UUaksHTMatchLiveFeedModel *feedMode in c_liveFeedModel) {
...
//        BJLog(@"quarter = %ld",quarter);
    ...
    UUaksHTMatchLiveFeedModel *last_feedMode = c_liveFeedModel.lastObject;
