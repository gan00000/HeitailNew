...
@property (nonatomic) SundayHTMatchHomeModel *matchModel;
...
- (void)setupViews {
    ...
    [self.imContentTableView registerNib:[UINib nibWithNibName:NSStringFromClass([HourseHTChatContentCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([HourseHTChatContentCell class])];
    
    [self.imContentTableView registerNib:[UINib nibWithNibName:NSStringFromClass([YYPackageHTChatSelfContentCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([YYPackageHTChatSelfContentCell class])];
    
    //    kWeakSelf
    //    self.imContentTableView.mj_header = [HourseMJRefreshGenerator bj_headerWithRefreshingBlock:^{
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket{
    ...
    if (self.socket != nil) {
        ...
        if (_socket.readyState == SR_OPEN) {
            ...
            //用户登录
            if (MMTodayHTUserManager.tao_userToken && ![@"" isEqualToString:MMTodayHTUserManager.tao_userToken]) {
                [self sendLoginData];
            }
...
-(void)sendLoginData
{
    ...
    req.token = MMTodayHTUserManager.tao_userToken;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (chatContent.fromUserId == [userInfoModel.user_id longLongValue]) {
        ...
        HourseHTChatContentCell *cell = (HourseHTChatContentCell *)[tableView dequeueReusableCellWithIdentifier:NSStringFromClass([YYPackageHTChatSelfContentCell class])];
    ...
    HourseHTChatContentCell *cell = (HourseHTChatContentCell *)[tableView dequeueReusableCellWithIdentifier:@"HourseHTChatContentCell"];
    //[cell setChaMsg:[NSString stringWithFormat:@"%@ - %@", chatContent.content, chatContent.fromUserName]];
    cell.chatContentLabel.text = chatContent.content;
    cell.userNameLabel.text = chatContent.fromUserName;
    [cell.userImage sd_setImageWithURL:[NSURL URLWithString:chatContent.fromUserImg] placeholderImage:HT_DEFAULT_TEAM_LOGO];
