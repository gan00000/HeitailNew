#import "SundayHTMatchDetailViewController.h"
#import "RRDogHTMatchWordLiveViewController.h"
#import "PXFunHTMatchCompareViewController.h"
#import "KMonkeyHTMatchDashboardViewController.h"
...
#import "LMPlayer.h"
#import "CCCaseHTIndicatorView.h"
#import "RRDogHTIMViewController.h"
#import "MMTodayHTUserManager.h"
#import "RRDogHTMatchWebPlayControllerViewController.h"
...
@interface SundayHTMatchDetailViewController () <UIScrollViewDelegate, LMVideoPlayerDelegate>
...
@property (nonatomic, strong) NSArray<SundayHTMatchLiveFeedModel *> *liveFeedList;//对阵也会用到这数据
@property (nonatomic, strong) NDeskHTMatchSummaryModel *matchSummaryModel;
@property (nonatomic, strong) PXFunHTMatchCompareModel *matchCompareModel;

@property (nonatomic,strong) RRDogHTIMViewController *imVc;

@property (nonatomic, strong) HourseHTMacthLivePostModel *livePost;
...
@property (nonatomic, strong) GGCatLMVideoPlayer *player;
@property (nonatomic, strong) MMTodayLMPlayerModel *playerModel;
/** 离开页面时候是否在播放 */
...
@property (nonatomic, strong)CCCaseHTIndicatorView *likeIndicatorView;
...
{
    NSLog(@"SundayHTMatchDetailViewController viewDidAppear");
    [IQKeyboardManager sharedManager].enable = NO;
}

- (void)viewDidDisappear:(BOOL)animated
{
    NSLog(@"SundayHTMatchDetailViewController viewDidDisappear");
...
- (void)dealloc {
    ...
    [self destoryPlayer];
    [MMTodayHTUserManager manager].matchSummaryModel = nil;
}
...
#pragma mark - private
- (void)setUpPlayerView:(MMTodayLMPlayerModel *) model {
    ...
    GGCatLMVideoPlayer *player = [GGCatLMVideoPlayer videoPlayerWithView:self.playerContentView delegate:self playerModel:model];
    self.player = player;
}
...
//    self.topDetailView.hidden = YES;
    ...
    //讚部分
    self.likeIndicatorView = [[CCCaseHTIndicatorView alloc] init];
...
- (IBAction)rightLikeBtnClick:(id)sender {//主隊
    ...
    [PXFunHTMatchSummaryRequest requestLikeMatchTeamWithGameId:self.matchModel.game_id type:@"1" successBlock:^(NDeskHTLikeTeamModel *m) {
        if (!m) {
           return;
        }
...
//    }
    ...
    [PXFunHTMatchSummaryRequest requestLikeMatchTeamWithGameId:self.matchModel.game_id type:@"2" successBlock:^(NDeskHTLikeTeamModel *m) {
        if (!m) {
            return;
        }
...
//    MMTodayLMPlayerModel *model = [[MMTodayLMPlayerModel alloc] init];
...
- (void)initData {
    ...
    if ([self.matchModel.scheduleStatus isEqualToString:@"InProgress"]) {
        ...
        [PXFunHTMatchSummaryRequest requestLivePostWithGameId:self.matchModel.game_id successBlock:^(HourseHTMacthLivePostModel *livePost) {
            if (livePost) {
...
- (void)loadData {
    ...
    self.error = nil;
    [PXFunHTMatchLiveFeedRequest taorequestLiveFeedWithGameId:self.matchModel.game_id successBlock:^(NSArray<SundayHTMatchLiveFeedModel *> *feedList) {
    ...
    [PXFunHTMatchSummaryRequest taorequestSummaryWithGameId:self.matchModel.game_id successBlock:^(NDeskHTMatchSummaryModel *summaryModel, PXFunHTMatchCompareModel *compareModel) {
       //summaryModel.scheduleStatus = @"InProgress";
        ...
        [MMTodayHTUserManager manager].matchSummaryModel = self.matchSummaryModel;
...
//                 YYPackageHTMatchVideoLiveViewController *detailVc =  self.loadedControllersArray[index];//[YYPackageHTMatchVideoLiveViewController taoviewController];
//                 detailVc.game_id = self.matchModel.game_id;
                 YYPackageHTHighLightsViewController *hlVc =  self.loadedControllersArray[index];
                 ...
                 RRDogHTMatchWordLiveViewController *wordVc = self.loadedControllersArray[index];
                [wordVc taorefreshWithLiveFeedList:self.liveFeedList summary:self.matchSummaryModel gameId:self.matchModel.game_id];
                
                 
             }else if (index == 2) {//對陣
                 PXFunHTMatchCompareViewController *compareVc = self.loadedControllersArray[index];
                 ...
                 RRDogHTIMViewController *imVc = self.loadedControllersArray[index];
                 [imVc setData:self.matchModel summary:self.matchSummaryModel];
             } else {
                 KMonkeyHTMatchDashboardViewController *dashbdVc = self.loadedControllersArray[index];
         ...
         if (index == 0) {
             ...
             //视频直播
             YYPackageHTHighLightsViewController *hlVc = [YYPackageHTHighLightsViewController taoviewController];
             [hlVc refreshDataWithGameId:self.matchModel.game_id mMatchSummaryModel:self.matchSummaryModel mCompareModel:self.matchCompareModel];
             vc = hlVc;
             ...
             RRDogHTMatchWordLiveViewController *wordVc = [RRDogHTMatchWordLiveViewController taoviewController];
            ...
            [wordVc taorefreshWithLiveFeedList:self.liveFeedList summary:self.matchSummaryModel gameId:self.matchModel.game_id];
             ...
             PXFunHTMatchCompareViewController *compareVc = [PXFunHTMatchCompareViewController taoviewController];
             ...
             self.imVc = [RRDogHTIMViewController taoviewController];
             [self.imVc setData:self.matchModel summary:self.matchSummaryModel];
             vc = self.imVc;
         } else {
             KMonkeyHTMatchDashboardViewController *dashboardVc = [KMonkeyHTMatchDashboardViewController taoviewController];
