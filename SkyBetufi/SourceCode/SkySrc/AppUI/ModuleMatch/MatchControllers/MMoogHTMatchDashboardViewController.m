...
@interface MMoogHTMatchDashboardViewController () <UIScrollViewDelegate>
...
@property (nonatomic, weak) WSKggHTMatchCompareModel *compareModel;
@end
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    if ([self.loadedFlagArray[index] boolValue]) {
        MMoogHTMatchSubDsbdViewController *vc = self.loadedControllersArray[index];
        if (index == 0) {
    ...
    MMoogHTMatchSubDsbdViewController *vc = [MMoogHTMatchSubDsbdViewController taoviewController];
    if (index == 0) {
        [vc taorefreshWithDetailList:self.compareModel.awayTeamDetails];
    } else if (index == 1) {
        [vc taorefreshWithDetailList:self.compareModel.homeTeamDetails];
