//
//  MMTodayPlayerInfoViewController.m
//  SkyBallPlay
//
...
@property (nonatomic) SundayHTMatchDetailsModel *model_1;
@property (nonatomic) SundayHTMatchDetailsModel *model_2;
@property (nonatomic) SundayHTMatchDetailsModel *model_3;
@property (nonatomic) SundayHTMatchDetailsModel *model_4;

@property (nonatomic) SundayHTMatchDetailsModel *model_all;

@property(weak,nonatomic) SundayHTMatchDetailsModel *model;
...
- (void)viewDidLoad {
    ...
    self.title = @"數據卡";
    NSLog(@"MMTodayPlayerInfoViewController viewDidLoad");
...
//    UIBarButtonItem *saveUIBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"nav_icon_share"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] style:UIBarButtonItemStylePlain target:self action:@selector(onShareButtonTapped:)];
    ...
    self.model_all = [[SundayHTMatchDetailsModel alloc] init];
    
    [kWindow showLoadingView];
    [PXFunHTMatchSummaryRequest requestPlayerInfo:self.model.gameId teamId:self.model.teamId playerId:self.model.playerId successBlock:^(NSArray<SundayHTMatchDetailsModel *> *model) {
        ...
        for (SundayHTMatchDetailsModel *xm in model) {
    ...
    NDeskHTMatchSummaryModel *matchSummaryModel = [MMTodayHTUserManager manager].matchSummaryModel;
...
-(void)setPlayerData:(SundayHTMatchDetailsModel *)model
{
...
-(void)setData:(SundayHTMatchDetailsModel *)model{
    NSLog(@"MMTodayPlayerInfoViewController data");
    self.model = model;
...
- (void)share {
    kWeakSelf
    [YYPackageHTLoginAlertView taoshowShareAlertViewWithFBAndSave:^(HTLoginPlatform platform) {
...
- (void)doShareToPlatform:(UMSocialPlatformType)platformType withMessage:(UMSocialMessageObject *)messageObject {
    [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:[PXFunPPXXBJViewControllerCenter currentViewController] completion:^(id result, NSError *error) {
        BJLog(@"result = %@", error);
    }];
}
...
{
    kWeakSelf
    [PXFunHTMatchSummaryRequest getShootPointWithGameId:self.model.gameId home_away:@"" playerId:self.model.playerId quarter:quartId successBlock:^(NSArray<HourseHotShootPointModel *> *model) {
...
-(void)addPointToViewWithData:(NSArray<HourseHotShootPointModel *> *)model
{
    ...
    CGFloat xxHeight = self.hotPointView.height;
        ...
        for (HourseHotShootPointModel *m in model) { //X = 49 Y = 32
