#import "KMonkeyHTMatchSubDsbdViewController.h"
#import "PXFunHTMatchDataLeftCell.h"
#import "GGCatHTMatchDataRightCell.h"
#import "MMTodayPlayerInfoViewController.h"

@interface KMonkeyHTMatchSubDsbdViewController ()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
@property (weak, nonatomic) IBOutlet UITableView *leftTableView;
@property (weak, nonatomic) IBOutlet UIScrollView *scrollContentView;
@property (nonatomic, strong) UITableView *rightTableView;
@property (nonatomic) NSMutableArray<SundayHTMatchDetailsModel *> *dataList;

@property (nonatomic) NSArray<SundayHTMatchDetailsModel *> *tempDetailList;
...
- (void)setupLeftTableView {
    ...
    [self.leftTableView registerNib:[UINib nibWithNibName:NSStringFromClass([PXFunHTMatchDataLeftCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([PXFunHTMatchDataLeftCell class])];
}
- (void)setupRightTableView {
    ...
    [self.rightTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GGCatHTMatchDataRightCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([GGCatHTMatchDataRightCell class])];
    [self.scrollContentView addSubview:self.rightTableView];
}
- (void)taorefreshWithDetailList:(NSArray<SundayHTMatchDetailsModel *> *)detailList {
    ...
    [firstPlayerArr sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        SundayHTMatchDetailsModel *model1 = (SundayHTMatchDetailsModel *)obj1;
        SundayHTMatchDetailsModel *model2 = (SundayHTMatchDetailsModel *)obj2;
...
- (void)countWithDataList:(NSArray<SundayHTMatchDetailsModel *> *)dataList { //计算总计
    if (dataList.count == 0) {
        return;
    }
    if (!self.counts) {
    ...
    NSInteger blk = 0;
    for (SundayHTMatchDetailsModel *dmodel in dataList) {
    ...
    //添加一个命中率
    SundayHTMatchDetailsModel *xxModel = [[SundayHTMatchDetailsModel alloc] init];
    xxModel.name = @"命中率";
    xxModel.time = @"-";
    xxModel.pts = @"-";
    xxModel.ast = @"-";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SundayHTMatchDetailsModel *model = self.dataList[indexPath.row];
    if (tableView == self.leftTableView) {
        PXFunHTMatchDataLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([PXFunHTMatchDataLeftCell class])];
        [cell taorefreshWithName:model.name row:indexPath.row];
        return cell;
    }
    GGCatHTMatchDataRightCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([GGCatHTMatchDataRightCell class])];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.leftTableView) {
        ...
        MMTodayPlayerInfoViewController *piVc = [MMTodayPlayerInfoViewController taoviewController];
        [piVc setData:model];
