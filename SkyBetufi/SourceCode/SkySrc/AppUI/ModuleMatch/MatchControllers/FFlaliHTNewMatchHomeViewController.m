...
@interface FFlaliHTNewMatchHomeViewController () <UIScrollViewDelegate>
...
@implementation FFlaliHTNewMatchHomeViewController
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if ([self.loadedFlagArray[index] boolValue]) {
        if (index == 0) {
            ...
            BByasHTMatchHomeViewController *vc = self.loadedControllersArray[index];
    ...
    if (index == 0) {
        ...
        BByasHTMatchHomeViewController *mhVc = [BByasHTMatchHomeViewController taoviewController];
