...
@interface NSNiceHTNewMatchHomeViewController () <UIScrollViewDelegate>
...
@implementation NSNiceHTNewMatchHomeViewController


+ (instancetype)taoviewController {
    return kLoadStoryboardWithName(@"FaCaiHTNewMatchHome");
}
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if ([self.loadedFlagArray[index] boolValue]) {
        if (index == 0) {
            ...
            BlysaHTMatchHomeViewController *vc = self.loadedControllersArray[index];
    ...
    if (index == 0) {
        ...
        BlysaHTMatchHomeViewController *mhVc = [BlysaHTMatchHomeViewController taoviewController];
