//
//  MMoogPlayerInfoViewController.m
//  SkyBallPlay
//
...
@interface MMoogPlayerInfoViewController ()
@property (weak, nonatomic) IBOutlet UIImageView *iconImageView;
@property (weak, nonatomic) IBOutlet UILabel *playerNameLabel;
@property (weak, nonatomic) IBOutlet UILabel *teamLabel;
@property (weak, nonatomic) IBOutlet HMSegmentedControl *segmentControl;

@property (nonatomic) TuTuosHTMatchDetailsModel *model_1;
@property (nonatomic) TuTuosHTMatchDetailsModel *model_2;
@property (nonatomic) TuTuosHTMatchDetailsModel *model_3;
@property (nonatomic) TuTuosHTMatchDetailsModel *model_4;

@property (nonatomic) TuTuosHTMatchDetailsModel *model_all;

@property(weak,nonatomic) TuTuosHTMatchDetailsModel *model;
...
//    UIBarButtonItem *saveUIBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"nav_icon_share"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] style:UIBarButtonItemStylePlain target:self action:@selector(onShareButtonTapped:)];
    ...
    self.model_all = [[TuTuosHTMatchDetailsModel alloc] init];
    
    [kWindow showLoadingView];
    [MMoogHTMatchSummaryRequest requestPlayerInfo:self.model.gameId teamId:self.model.teamId playerId:self.model.playerId successBlock:^(NSArray<TuTuosHTMatchDetailsModel *> *model) {
        ...
        for (TuTuosHTMatchDetailsModel *xm in model) {
    ...
    NSNiceHTMatchSummaryModel *matchSummaryModel = [NSNiceHTUserManager manager].matchSummaryModel;
...
-(void)setPlayerData:(TuTuosHTMatchDetailsModel *)model
{
...
-(void)setData:(TuTuosHTMatchDetailsModel *)model{
    NSLog(@"MMoogPlayerInfoViewController data");
    self.model = model;
...
- (void)share {
    kWeakSelf
    [TuTuosHTLoginAlertView taoshowShareAlertViewWithFBAndSave:^(HTLoginPlatform platform) {
...
- (void)doShareToPlatform:(UMSocialPlatformType)platformType withMessage:(UMSocialMessageObject *)messageObject {
    [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:[UUaksPPXXBJViewControllerCenter currentViewController] completion:^(id result, NSError *error) {
        BJLog(@"result = %@", error);
    }];
}
...
{
    kWeakSelf
    [MMoogHTMatchSummaryRequest getShootPointWithGameId:self.model.gameId home_away:@"" playerId:self.model.playerId quarter:quartId successBlock:^(NSArray<BByasHotShootPointModel *> *model) {
...
-(void)addPointToViewWithData:(NSArray<BByasHotShootPointModel *> *)model
{
    ...
    CGFloat xxHeight = self.hotPointView.height;
        ...
        for (BByasHotShootPointModel *m in model) { //X = 49 Y = 32
