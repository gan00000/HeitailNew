...
@property (nonatomic) UUaksHTMatchDetailsModel *model_1;
@property (nonatomic) UUaksHTMatchDetailsModel *model_2;
@property (nonatomic) UUaksHTMatchDetailsModel *model_3;
@property (nonatomic) UUaksHTMatchDetailsModel *model_4;

@property (nonatomic) UUaksHTMatchDetailsModel *model_all;

@property(weak,nonatomic) UUaksHTMatchDetailsModel *model;
...
//    UIBarButtonItem *saveUIBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"gurk_nav_icon_share"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] style:UIBarButtonItemStylePlain target:self action:@selector(onShareButtonTapped:)];
    ...
    self.model_all = [[UUaksHTMatchDetailsModel alloc] init];
    
    [kWindow showLoadingView];
    [MMoogHTMatchSummaryRequest requestPlayerInfo:self.model.gameId teamId:self.model.teamId playerId:self.model.playerId successBlock:^(NSArray<UUaksHTMatchDetailsModel *> *model) {
        ...
        for (UUaksHTMatchDetailsModel *xm in model) {
    ...
    KSasxHTMatchSummaryModel *matchSummaryModel = [TuTuosHTUserManager manager].matchSummaryModel;
...
-(void)setPlayerData:(UUaksHTMatchDetailsModel *)model
{
...
-(void)setData:(UUaksHTMatchDetailsModel *)model{
...
- (void)share {
    kWeakSelf
    [MMoogHTLoginAlertView taoshowShareAlertViewWithFBAndSave:^(HTLoginPlatform platform) {
...
- (void)doShareToPlatform:(UMSocialPlatformType)platformType withMessage:(UMSocialMessageObject *)messageObject {
    [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:[CfipyPPXXBJViewControllerCenter currentViewController] completion:^(id result, NSError *error) {
        BJLog(@"result = %@", error);
    }];
}
...
{
    kWeakSelf
    [MMoogHTMatchSummaryRequest getShootPointWithGameId:self.model.gameId home_away:@"" playerId:self.model.playerId quarter:quartId successBlock:^(NSArray<TuTuosHotShootPointModel *> *model) {
...
-(void)addPointToViewWithData:(NSArray<TuTuosHotShootPointModel *> *)model
{
