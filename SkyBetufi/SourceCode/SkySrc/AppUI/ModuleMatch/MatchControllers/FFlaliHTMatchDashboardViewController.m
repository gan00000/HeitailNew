...
@interface FFlaliHTMatchDashboardViewController () <UIScrollViewDelegate>
...
@property (nonatomic, weak) BByasHTMatchCompareModel *compareModel;
@end
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    if ([self.loadedFlagArray[index] boolValue]) {
        CfipyHTMatchSubDsbdViewController *vc = self.loadedControllersArray[index];
        if (index == 0) {
    ...
    CfipyHTMatchSubDsbdViewController *vc = [CfipyHTMatchSubDsbdViewController taoviewController];
    if (index == 0) {
        [vc taorefreshWithDetailList:self.compareModel.awayTeamDetails];
    } else if (index == 1) {
        [vc taorefreshWithDetailList:self.compareModel.homeTeamDetails];
