...
@interface TuTuosHTMatchDetailViewController () <UIScrollViewDelegate, LMVideoPlayerDelegate>
...
@property (nonatomic, strong) NSArray<UUaksHTMatchLiveFeedModel *> *liveFeedList;//对阵也会用到这数据
@property (nonatomic, strong) NSNiceHTMatchSummaryModel *matchSummaryModel;
@property (nonatomic, strong) WSKggHTMatchCompareModel *matchCompareModel;

@property (nonatomic,strong) MMoogHTIMViewController *imVc;

@property (nonatomic, strong) NSNiceHTMacthLivePostModel *livePost;
...
@property (nonatomic, strong) WSKggLMVideoPlayer *player;
...
@property (nonatomic, strong)WSKggHTIndicatorView *likeIndicatorView;
...
@end
@implementation TuTuosHTMatchDetailViewController
...
{
    NSLog(@"TuTuosHTMatchDetailViewController viewDidAppear");
    [IQKeyboardManager sharedManager].enable = NO;
}

- (void)viewDidDisappear:(BOOL)animated
{
    NSLog(@"TuTuosHTMatchDetailViewController viewDidDisappear");
...
- (void)dealloc {
    ...
    [self destoryPlayer];
    [NSNiceHTUserManager manager].matchSummaryModel = nil;
}
...
- (void)setUpPlayerView:(BlysaLMPlayerModel *) model {
    ...
    WSKggLMVideoPlayer *player = [WSKggLMVideoPlayer videoPlayerWithView:self.playerContentView delegate:self playerModel:model];
    self.player = player;
}
...
//    self.topDetailView.hidden = YES;
    ...
    //讚部分
    self.likeIndicatorView = [[WSKggHTIndicatorView alloc] init];
...
- (IBAction)rightLikeBtnClick:(id)sender {//主隊
    ...
    [MMoogHTMatchSummaryRequest requestLikeMatchTeamWithGameId:self.matchModel.game_id type:@"1" successBlock:^(YeYeeHTLikeTeamModel *m) {
        if (!m) {
           return;
        }
...
//    }
    ...
    [MMoogHTMatchSummaryRequest requestLikeMatchTeamWithGameId:self.matchModel.game_id type:@"2" successBlock:^(YeYeeHTLikeTeamModel *m) {
        if (!m) {
            return;
        }
...
//    }];
    ...
    token = [NSNiceHTUserManager tao_userToken];
...
- (void)initData {
    ...
    if ([self.matchModel.scheduleStatus isEqualToString:@"InProgress"]) {
        ...
        [MMoogHTMatchSummaryRequest requestLivePostWithGameId:self.matchModel.game_id successBlock:^(NSNiceHTMacthLivePostModel *livePost) {
            if (livePost) {
...
- (void)loadData {
    ...
    self.error = nil;
    [YeYeeHTMatchLiveFeedRequest taorequestLiveFeedWithGameId:self.matchModel.game_id successBlock:^(NSArray<UUaksHTMatchLiveFeedModel *> *feedList) {
    ...
    [MMoogHTMatchSummaryRequest taorequestSummaryWithGameId:self.matchModel.game_id successBlock:^(NSNiceHTMatchSummaryModel *summaryModel, WSKggHTMatchCompareModel *compareModel) {
       //summaryModel.scheduleStatus = @"InProgress";
        ...
        [NSNiceHTUserManager manager].matchSummaryModel = self.matchSummaryModel;
...
//                 WSKggHTMatchVideoLiveViewController *detailVc =  self.loadedControllersArray[index];//[WSKggHTMatchVideoLiveViewController taoviewController];
//                 detailVc.game_id = self.matchModel.game_id;
                 FFlaliHTHighLightsViewController *hlVc =  self.loadedControllersArray[index];
                 ...
                 UUaksHTMatchWordLiveViewController *wordVc = self.loadedControllersArray[index];
                [wordVc taorefreshWithLiveFeedList:self.liveFeedList summary:self.matchSummaryModel gameId:self.matchModel.game_id];
                
                 
             }else if (index == 2) {//對陣
                 WSKggHTMatchCompareViewController *compareVc = self.loadedControllersArray[index];
                 ...
                 MMoogHTIMViewController *imVc = self.loadedControllersArray[index];
                 [imVc setData:self.matchModel summary:self.matchSummaryModel];
             } else {
                 MMoogHTMatchDashboardViewController *dashbdVc = self.loadedControllersArray[index];
         ...
         if (index == 0) {
             ...
             //视频直播
             FFlaliHTHighLightsViewController *hlVc = [FFlaliHTHighLightsViewController taoviewController];
             [hlVc refreshDataWithGameId:self.matchModel.game_id mMatchSummaryModel:self.matchSummaryModel mCompareModel:self.matchCompareModel];
             vc = hlVc;
             ...
             UUaksHTMatchWordLiveViewController *wordVc = [UUaksHTMatchWordLiveViewController taoviewController];
            ...
            [wordVc taorefreshWithLiveFeedList:self.liveFeedList summary:self.matchSummaryModel gameId:self.matchModel.game_id];
             ...
             WSKggHTMatchCompareViewController *compareVc = [WSKggHTMatchCompareViewController taoviewController];
             ...
             self.imVc = [MMoogHTIMViewController taoviewController];
             [self.imVc setData:self.matchModel summary:self.matchSummaryModel];
             vc = self.imVc;
         } else {
             MMoogHTMatchDashboardViewController *dashboardVc = [MMoogHTMatchDashboardViewController taoviewController];
...
-(BOOL)showPlayback
{
    ...
    NSString *timeStamp = [WSKggConfigCoreUtil getTimeStrWithString: timeStr];
    
    double now_timestamp = [[WSKggConfigCoreUtil getTimeStamp] doubleValue];
