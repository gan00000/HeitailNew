...
@interface NDeskHTNewMatchHomeViewController () <UIScrollViewDelegate>
...
@implementation NDeskHTNewMatchHomeViewController


+ (instancetype)taoviewController {
    return kLoadStoryboardWithName(@"GlodBuleHTNewMatchHome");
}
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if ([self.loadedFlagArray[index] boolValue]) {
        if (index == 0) {
            ...
            YYPackageHTMatchHomeViewController *vc = self.loadedControllersArray[index];
    ...
    if (index == 0) {
        ...
        YYPackageHTMatchHomeViewController *mhVc = [YYPackageHTMatchHomeViewController taoviewController];
