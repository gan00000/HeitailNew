...
@interface KMonkeyHTMatchDashboardViewController () <UIScrollViewDelegate>
...
@property (nonatomic, weak) PXFunHTMatchCompareModel *compareModel;
@end
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    if ([self.loadedFlagArray[index] boolValue]) {
        KMonkeyHTMatchSubDsbdViewController *vc = self.loadedControllersArray[index];
        if (index == 0) {
    ...
    KMonkeyHTMatchSubDsbdViewController *vc = [KMonkeyHTMatchSubDsbdViewController taoviewController];
    if (index == 0) {
        [vc taorefreshWithDetailList:self.compareModel.awayTeamDetails];
    } else if (index == 1) {
        [vc taorefreshWithDetailList:self.compareModel.homeTeamDetails];
