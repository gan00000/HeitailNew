...
@interface GlodBuleHTMatchDashboardViewController () <UIScrollViewDelegate>
...
@property (nonatomic, weak) GlodBuleHTMatchCompareModel *compareModel;
@end
@implementation GlodBuleHTMatchDashboardViewController
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    if ([self.loadedFlagArray[index] boolValue]) {
        GlodBuleHTMatchSubDsbdViewController *vc = self.loadedControllersArray[index];
        if (index == 0) {
            [vc taorefreshWithDetailList:self.compareModel.awayTeamDetails];
        } else if (index == 1) {
            [vc taorefreshWithDetailList:self.compareModel.homeTeamDetails];
        }
        return;
    }
    GlodBuleHTMatchSubDsbdViewController *vc = [GlodBuleHTMatchSubDsbdViewController taoviewController];
    if (index == 0) {
        [vc taorefreshWithDetailList:self.compareModel.awayTeamDetails];
    } else if (index == 1) {
        [vc taorefreshWithDetailList:self.compareModel.homeTeamDetails];
