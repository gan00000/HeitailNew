...
#import "MMoogHTMatchHomeRequest.h"
#import "BByasHTMatchHomeCell.h"
#import "WSKggHTMatchHomeGroupHeaderView.h"
#import "KSasxHTDatePickerView.h"
#import "NSDateFormatter+KSasxCCCaseDRExtension.h"
#import "UIView+FFlaliHourseLoading.h"
#import "UIView+BByasPXFunEmptyView.h"
#import "AppDelegate.h"
#import "WSKggConfigCoreUtil.h"
#import "KSasxHTMatchHomeCell222.h"

@interface BlysaHTMatchHomeViewController () <UITableViewDelegate, UITableViewDataSource>
...
@end
@implementation BlysaHTMatchHomeViewController
...
- (IBAction)onPreviousButtonTapped:(UIButton *)sender {
    ...
    [NSNiceBJLoadingHud showHUDInView:self.view];
}
...
- (IBAction)onSelectDateButtonTapped:(id)sender {
    kWeakSelf
    [KSasxHTDatePickerView taoshowWithWithDate:self.startDate didTapEnterBlock:^BOOL(NSDate *date) {
        weakSelf.startDate = date;
        [NSNiceBJLoadingHud showHUDInView:weakSelf.view];
        return YES;
    }];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    UUaksHTMatchHomeGroupModel *groupModel = self.matchList[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (isAppInView) {
        ...
        UUaksHTMatchHomeGroupModel *groupModel = self.matchList[indexPath.section];
        [cell taosetupWithMatchModel:groupModel.matchList[indexPath.row] matchType:self.matchType];
        return cell;
    }
    
    BByasHTMatchHomeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"BByasHTMatchHomeCell"];
    UUaksHTMatchHomeGroupModel *groupModel = self.matchList[indexPath.section];
    [cell taosetupWithMatchModel:groupModel.matchList[indexPath.row] matchType:self.matchType];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //用户登录
//    if (!NSNiceHTUserManager.tao_userToken || [@"" isEqualToString:NSNiceHTUserManager.tao_userToken]) {
//        [kWindow showToast:@"請先登入帳號"];
//        return;
//    }
    
    UUaksHTMatchHomeGroupModel *groupModel = self.matchList[indexPath.section];
    TuTuosHTMatchDetailViewController *detailVc = [TuTuosHTMatchDetailViewController taoviewController];
    detailVc.matchModel = groupModel.matchList[indexPath.row];
    [self.navigationController pushViewController:detailVc animated:YES];
    
    [WSKggConfigCoreUtil share].matchType = self.matchType;
}
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (self.matchList.count == 0) {
        return nil;
    }
    WSKggHTMatchHomeGroupHeaderView *view = (WSKggHTMatchHomeGroupHeaderView *)[tableView dequeueReusableHeaderFooterViewWithIdentifier:@"WSKggHTMatchHomeGroupHeaderView"];
    UUaksHTMatchHomeGroupModel *groupModel = self.matchList[section];
...
//    self.title = @"比賽";
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"BByasHTMatchHomeCell" bundle:nil]
         forCellReuseIdentifier:@"BByasHTMatchHomeCell"];
    [self.tableView registerNib:[UINib nibWithNibName:@"WSKggHTMatchHomeGroupHeaderView" bundle:nil] forHeaderFooterViewReuseIdentifier:@"WSKggHTMatchHomeGroupHeaderView"];
...
- (void)refreshUI {
    ...
    [NSNiceBJLoadingHud hideHUDInView:self.view];
    [self refreshTimeTitle];
...
- (void)loadDataType:(NSInteger)type {
    ...
    self.requesting = YES;
    [MMoogHTMatchHomeRequest taorequestWithStartDate:[self ymdWithDate:self.loadStartDate]
                                     endDate:[self ymdWithDate:self.loadEndDate]
                                         competition_id:self.matchType
                                successBlock:^(NSArray<UUaksHTMatchHomeGroupModel *> *matchList,NSArray<FFlaliHTMatchHomeModel *> *matchA) {
...
//                                        [self.tableView showEmptyView];
                                        ...
                                        NSMutableArray<UUaksHTMatchHomeGroupModel *> *tempMatchList = [NSMutableArray array];
                                        for (UUaksHTMatchHomeGroupModel *groupModel in matchList) {
                                            for (FFlaliHTMatchHomeModel *model in groupModel.matchList) {
                                        ...
                                        if (self.inProgressMatchs.count == 0) {
                                            ...
                                            for (FFlaliHTMatchHomeModel *model in self.inProgressMatchs.allValues) {//加载正在比赛中的数据
                                                [MMoogHTMatchHomeRequest taorequestMatchProgressWithGameId:model.game_id successBlock:^(NSString *game_id, NSString *quarter, NSString *time) {
                                                    FFlaliHTMatchHomeModel *matchModel = [self.inProgressMatchs objectForKey:game_id];
