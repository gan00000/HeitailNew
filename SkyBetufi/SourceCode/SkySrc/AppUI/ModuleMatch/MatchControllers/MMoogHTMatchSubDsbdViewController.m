#import "MMoogHTMatchSubDsbdViewController.h"
#import "UUaksHTMatchDataLeftCell.h"
#import "YeYeeHTMatchDataRightCell.h"
#import "MMoogPlayerInfoViewController.h"

@interface MMoogHTMatchSubDsbdViewController ()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
@property (weak, nonatomic) IBOutlet UITableView *leftTableView;
@property (weak, nonatomic) IBOutlet UIScrollView *scrollContentView;
@property (nonatomic, strong) UITableView *rightTableView;
@property (nonatomic) NSMutableArray<TuTuosHTMatchDetailsModel *> *dataList;

@property (nonatomic) NSArray<TuTuosHTMatchDetailsModel *> *tempDetailList;
...
- (void)setupLeftTableView {
    ...
    [self.leftTableView registerNib:[UINib nibWithNibName:NSStringFromClass([UUaksHTMatchDataLeftCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([UUaksHTMatchDataLeftCell class])];
}
- (void)setupRightTableView {
    ...
    [self.rightTableView registerNib:[UINib nibWithNibName:NSStringFromClass([YeYeeHTMatchDataRightCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([YeYeeHTMatchDataRightCell class])];
    [self.scrollContentView addSubview:self.rightTableView];
}
- (void)taorefreshWithDetailList:(NSArray<TuTuosHTMatchDetailsModel *> *)detailList {
    ...
    [firstPlayerArr sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        TuTuosHTMatchDetailsModel *model1 = (TuTuosHTMatchDetailsModel *)obj1;
        TuTuosHTMatchDetailsModel *model2 = (TuTuosHTMatchDetailsModel *)obj2;
...
- (void)countWithDataList:(NSArray<TuTuosHTMatchDetailsModel *> *)dataList { //计算总计
    if (dataList.count == 0) {
        return;
    }
    if (!self.counts) {
    ...
    NSInteger blk = 0;
    for (TuTuosHTMatchDetailsModel *dmodel in dataList) {
    ...
    //添加一个命中率
    TuTuosHTMatchDetailsModel *xxModel = [[TuTuosHTMatchDetailsModel alloc] init];
    xxModel.name = @"命中率";
    xxModel.time = @"-";
    xxModel.pts = @"-";
    xxModel.ast = @"-";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TuTuosHTMatchDetailsModel *model = self.dataList[indexPath.row];
    if (tableView == self.leftTableView) {
        UUaksHTMatchDataLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([UUaksHTMatchDataLeftCell class])];
        [cell taorefreshWithName:model.name row:indexPath.row];
        return cell;
    }
    YeYeeHTMatchDataRightCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([YeYeeHTMatchDataRightCell class])];
