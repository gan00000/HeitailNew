...
@interface NSNiceHTMainPageHomeCell () <WKNavigationDelegate, YSPlayerControllerDelegate>
...
//
//@property (nonatomic, strong) WSKggLMVideoPlayer *ijkPlayer;
...
@end
@implementation NSNiceHTMainPageHomeCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (IBAction)addSave:(UIButton *)sender {
    ...
    if (sender.selected) {
        [TuTuosHTUserRequest taodeleteLikeWithNewsId:self.newsModel.news_id successBlock:^{
        ...
        [TuTuosHTUserRequest taoaddLikeWithNewsId:self.newsModel.news_id successBlock:^{
...
//    [self.webContentView removeAllSubView];
    ...
    self.playerController = [[UUaksYSPlayerController alloc] initWithContentMediaInfo: self.newsModel.plMediaInfo];
...
- (void)playerViewEnterFullScreen:(UUaksYSPlayerController *)playerController{
...
- (void)playerViewExitFullScreen:(UUaksYSPlayerController *)playerController {
