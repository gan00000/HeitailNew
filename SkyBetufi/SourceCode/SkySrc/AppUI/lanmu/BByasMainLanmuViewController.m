...
@interface BByasMainLanmuViewController () <UIScrollViewDelegate>
...
@implementation BByasMainLanmuViewController
...
//    [MMoogHTMatchHomeRequest get_league_list_successBlock:^(NSDictionary *leagueList) {
//
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if ([self.loadedFlagArray[index] boolValue]) {
        if (index == 0) {
            ...
            BlysaHTNewsHomeViewController *vc = self.loadedControllersArray[index];
//            vc.matchType = self.titlesArrayValue[index];
        } else if (index == 1) {
      
            TuTuosHTFilmHomeViewController *vc = self.loadedControllersArray[index];
//            vc.matchType = self.titlesArrayValue[index];
        }else if (index == 2) {
            YeYeeHTDataHomeViewController *vc = self.loadedControllersArray[index];
        }
        return;
    }
    kWeakSelf
    ...
    if (index == 0) {
        ...
        BlysaHTNewsHomeViewController *mhVc = [BlysaHTNewsHomeViewController taoviewController];
//        mhVc.matchType = self.titlesArrayValue[index];
        vc = mhVc;

    } else if (index == 1) {
        TuTuosHTFilmHomeViewController *mhVc = [TuTuosHTFilmHomeViewController taoviewController];
//        mhVc.matchType = self.titlesArrayValue[index];
        vc = mhVc;
    }else if (index == 2) {
        YeYeeHTDataHomeViewController *mhVc = [YeYeeHTDataHomeViewController taoviewController];
