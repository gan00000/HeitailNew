...
@interface MainLanmuViewController () <UIScrollViewDelegate>
...
@implementation MainLanmuViewController
...
//    [GGCatHTMatchHomeRequest get_league_list_successBlock:^(NSDictionary *leagueList) {
//
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if ([self.loadedFlagArray[index] boolValue]) {
        if (index == 0) {
            ...
            NDeskHTNewsHomeViewController *vc = self.loadedControllersArray[index];
//            vc.matchType = self.titlesArrayValue[index];
        } else if (index == 1) {
      
            RRDogHTFilmHomeViewController *vc = self.loadedControllersArray[index];
//            vc.matchType = self.titlesArrayValue[index];
        }else if (index == 2) {
            MMTodayHTDataHomeViewController *vc = self.loadedControllersArray[index];
        }
        return;
    }
    kWeakSelf
    ...
    if (index == 0) {
        ...
        NDeskHTNewsHomeViewController *mhVc = [NDeskHTNewsHomeViewController taoviewController];
//        mhVc.matchType = self.titlesArrayValue[index];
        vc = mhVc;

    } else if (index == 1) {
        RRDogHTFilmHomeViewController *mhVc = [RRDogHTFilmHomeViewController taoviewController];
//        mhVc.matchType = self.titlesArrayValue[index];
        vc = mhVc;
    }else if (index == 2) {
        MMTodayHTDataHomeViewController *mhVc = [MMTodayHTDataHomeViewController taoviewController];
