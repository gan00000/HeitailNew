...
@interface NSNiceHTTabBarHomeViewController () <UITableViewDelegate, UITableViewDataSource>
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic, assign) NSInteger messageCount;
@end
@implementation NSNiceHTTabBarHomeViewController
+ (instancetype)taoviewController {
    return [[NSNiceHTTabBarHomeViewController alloc] init];
}
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"我的";
...
- (void)viewDidAppear:(BOOL)animated{
    NSLog(@"NSNiceHTTabBarHomeViewController viewDidAppear");
    [self.tableView reloadData];
}
- (void)dealloc {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 1) {
        UUaksHTMeCenterItemsCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([UUaksHTMeCenterItemsCell class])];
