...
@interface MMTodayHTUserManager () <LineSDKLoginDelegate,ASAuthorizationControllerDelegate,ASAuthorizationControllerPresentationContextProviding>
@property (nonatomic, strong) GGCatHTUserInfoModel *userInfoModel;
@property (nonatomic, strong) FBSDKLoginManager *tao_fbLoginManager;
@end
@implementation MMTodayHTUserManager
- (instancetype)init {
    self = [super init];
    if (self) {
        self.svgImageCache = [[NSMutableDictionary alloc] initWithCapacity:0];
        if ([MMTodayHTUserManager tao_isUserLogin]) {
            NSData *data = [NSData dataWithContentsOfFile:[MMTodayHTUserManager taouserInfoPath]];
            self.userInfoModel = [GGCatHTUserInfoModel yy_modelWithJSON:data];
        }
    }
    return self;
}
...
+ (instancetype)manager {
    static MMTodayHTUserManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[MMTodayHTUserManager alloc] init];
    });
    return manager;
}
+ (BOOL)tao_isUserLogin {
    return [MMTodayHTUserManager tao_userToken].length > 0;
}
+ (GGCatHTUserInfoModel *)tao_userInfo {
    return [MMTodayHTUserManager manager].userInfoModel;
}
+ (void)saveUserInfo:(NSDictionary *)userInfo {
    MMTodayHTUserManager *manager = [MMTodayHTUserManager manager];
    manager.userInfoModel = [GGCatHTUserInfoModel yy_modelWithJSON:userInfo];
    NSData *data = [userInfo yy_modelToJSONData];
    [data writeToFile:[MMTodayHTUserManager taouserInfoPath] atomically:YES];
...
+ (void)tao_doUserLogin {
    [YYPackageHTLoginAlertView taoshowLoginAlertViewWithSelectBlock:^(HTLoginPlatform platform) {
        MMTodayHTUserManager *manager = [MMTodayHTUserManager manager];
...
+ (void)tao_doUserLogout {
    ...
    [MMTodayDRSandBoxManager taodeleteFileAtPath:[self taouserInfoPath] doneBlock:^(NSString * _Nonnull filePath, BOOL success, NSError * _Nonnull error) {
        BJLog(@"用戶信息刪除 %d", success);
    }];
    [[NSNotificationCenter defaultCenter] postNotificationName:kUserLogStatusChagneNotice object:nil];
    [[MMTodayHTUserManager manager].tao_fbLoginManager logOut];
}
#pragma mark - Facebook Authory
- (void)getAuthWithUserInfoFromFacebook {
    
    kWeakSelf
    [self.tao_fbLoginManager logInWithPermissions:@[@"public_profile",@"email"] fromViewController:[PXFunPPXXBJViewControllerCenter currentViewController] handler:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
        if (error) {
...
//    [self.tao_fbLoginManager logInWithReadPermissions:@[@"public_profile",@"email"] fromViewController:[PXFunPPXXBJViewControllerCenter currentViewController] handler:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
//            if (error) {
//                [kWindow showToast:@"登錄失敗"];
//            } else if (result.isCancelled) {
//                [kWindow showToast:@"取消登錄"];
...
- (void)doLoginRequesWithAccessToken:(NSString *)accessToken sns:(NSInteger)sns {
    [HourseHTUserRequest taodoLoginRequestWithAccessToken:accessToken sns:sns successBlock:^(NSString * _Nonnull userToken) {
        [MMTodayHTUserManager saveUserToken:userToken];
        [MMTodayHTUserManager tao_refreshUserInfoWithSuccessBlock:nil];
...
- (void)doThirdLoginRequesWithAccessToken:(NSString *)accessToken sns:(NSInteger)sns userId:(NSString *)userId nickName:(NSString *)nickName email:(NSString *)email {
    ...
    [HourseHTUserRequest doThirdLoginRequestWithAccessToken:accessToken
                                                                sns:sns
                                                             userId:userId
                                                           nickName:nickName
                                                              email:email
...
+ (void)tao_refreshUserInfoWithSuccessBlock:(dispatch_block_t)block {
    [HourseHTUserRequest taorequestUserInfoWithSuccessBlock:^(NSDictionary * _Nonnull userInfo) {
...
- (void)handleGidSign{
    ...
    UIViewController *xxxxpresentingViewController = [KMonkeyBJUtility getCurrentViewController];
...
- (ASPresentationAnchor)presentationAnchorForAuthorizationController:(ASAuthorizationController *)controller API_AVAILABLE(ios(13.0)){
      ...
      // 返回window
      return [PXFunPPXXBJViewControllerCenter currentViewController].view.window;
}

#pragma mark - util
+ (NSString *)taouserInfoPath {
    return [NSString stringWithFormat:@"%@/userInfo.json", [MMTodayDRSandBoxManager taogetDocumentPath]];
}
...
+ (void)tao_showAlertWithTitle:(NSString *)title message:(NSString *)message cancelButton:(NSString *)cancelButton confirmButton:(NSString *)confirmButton confirmBlock:(dispatch_block_t)confirmBlock {
    ...
    [[PXFunPPXXBJViewControllerCenter currentViewController] presentViewController:alert animated:YES completion:nil];
}
