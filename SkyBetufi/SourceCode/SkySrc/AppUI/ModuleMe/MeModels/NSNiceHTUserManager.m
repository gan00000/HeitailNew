...
#import "TuTuosHTLoginAlertView.h"
#import "TuTuosHTUserRequest.h"
#import "UUaksDRSandBoxManager.h"
...
@interface NSNiceHTUserManager () <LineSDKLoginDelegate,ASAuthorizationControllerDelegate,ASAuthorizationControllerPresentationContextProviding>
@property (nonatomic, strong) TuTuosHTUserInfoModel *userInfoModel;
@property (nonatomic, strong) FBSDKLoginManager *tao_fbLoginManager;
@end
@implementation NSNiceHTUserManager
- (instancetype)init {
    self = [super init];
    if (self) {
        self.svgImageCache = [[NSMutableDictionary alloc] initWithCapacity:0];
        if ([NSNiceHTUserManager tao_isUserLogin]) {
            NSData *data = [NSData dataWithContentsOfFile:[NSNiceHTUserManager taouserInfoPath]];
            self.userInfoModel = [TuTuosHTUserInfoModel yy_modelWithJSON:data];
        }
    }
    return self;
}
...
+ (instancetype)manager {
    static NSNiceHTUserManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[NSNiceHTUserManager alloc] init];
    });
    return manager;
}
+ (BOOL)tao_isUserLogin {
    return [NSNiceHTUserManager tao_userToken].length > 0;
}
+ (TuTuosHTUserInfoModel *)tao_userInfo {
    return [NSNiceHTUserManager manager].userInfoModel;
}
+ (void)saveUserInfo:(NSDictionary *)userInfo {
    NSNiceHTUserManager *manager = [NSNiceHTUserManager manager];
    manager.userInfoModel = [TuTuosHTUserInfoModel yy_modelWithJSON:userInfo];
    NSData *data = [userInfo yy_modelToJSONData];
    [data writeToFile:[NSNiceHTUserManager taouserInfoPath] atomically:YES];
...
+ (void)tao_doUserLogin {
    [TuTuosHTLoginAlertView taoshowLoginAlertViewWithSelectBlock:^(HTLoginPlatform platform) {
        NSNiceHTUserManager *manager = [NSNiceHTUserManager manager];
...
+ (void)tao_doUserLogout {
    ...
    [UUaksDRSandBoxManager taodeleteFileAtPath:[self taouserInfoPath] doneBlock:^(NSString * _Nonnull filePath, BOOL success, NSError * _Nonnull error) {
        BJLog(@"用戶信息刪除 %d", success);
    }];
    [[NSNotificationCenter defaultCenter] postNotificationName:kUserLogStatusChagneNotice object:nil];
    [[NSNiceHTUserManager manager].tao_fbLoginManager logOut];
}
#pragma mark - Facebook Authory
- (void)getAuthWithUserInfoFromFacebook {
    
    kWeakSelf
    [self.tao_fbLoginManager logInWithPermissions:@[@"public_profile",@"email"] fromViewController:[UUaksPPXXBJViewControllerCenter currentViewController] handler:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
        if (error) {
...
//    [self.tao_fbLoginManager logInWithReadPermissions:@[@"public_profile",@"email"] fromViewController:[UUaksPPXXBJViewControllerCenter currentViewController] handler:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
//            if (error) {
//                [kWindow showToast:@"登錄失敗"];
//            } else if (result.isCancelled) {
//                [kWindow showToast:@"取消登錄"];
...
- (void)doLoginRequesWithAccessToken:(NSString *)accessToken sns:(NSInteger)sns {
    [TuTuosHTUserRequest taodoLoginRequestWithAccessToken:accessToken sns:sns successBlock:^(NSString * _Nonnull userToken) {
        [NSNiceHTUserManager saveUserToken:userToken];
        [NSNiceHTUserManager tao_refreshUserInfoWithSuccessBlock:nil];
...
- (void)doThirdLoginRequesWithAccessToken:(NSString *)accessToken sns:(NSInteger)sns userId:(NSString *)userId nickName:(NSString *)nickName email:(NSString *)email {
    ...
    [TuTuosHTUserRequest doThirdLoginRequestWithAccessToken:accessToken
                                                                sns:sns
                                                             userId:userId
                                                           nickName:nickName
                                                              email:email
...
+ (void)tao_refreshUserInfoWithSuccessBlock:(dispatch_block_t)block {
    [TuTuosHTUserRequest taorequestUserInfoWithSuccessBlock:^(NSDictionary * _Nonnull userInfo) {
...
- (void)handleGidSign{
    ...
    UIViewController *xxxxpresentingViewController = [UUaksBJUtility getCurrentViewController];
...
- (ASPresentationAnchor)presentationAnchorForAuthorizationController:(ASAuthorizationController *)controller API_AVAILABLE(ios(13.0)){
      ...
      // 返回window
      return [UUaksPPXXBJViewControllerCenter currentViewController].view.window;
}

#pragma mark - util
+ (NSString *)taouserInfoPath {
    return [NSString stringWithFormat:@"%@/userInfo.json", [UUaksDRSandBoxManager taogetDocumentPath]];
}
...
+ (void)tao_showAlertWithTitle:(NSString *)title message:(NSString *)message cancelButton:(NSString *)cancelButton confirmButton:(NSString *)confirmButton confirmBlock:(dispatch_block_t)confirmBlock {
    ...
    [[UUaksPPXXBJViewControllerCenter currentViewController] presentViewController:alert animated:YES completion:nil];
}
