...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ([GlodBuleHTUserManager tao_isUserLogin]) {
            NSData *data = [NSData dataWithContentsOfFile:[GlodBuleHTUserManager taouserInfoPath]];
...
+ (BOOL)tao_isUserLogin {
    return [GlodBuleHTUserManager tao_userToken].length > 0;
}
...
+ (void)saveUserInfo:(NSDictionary *)userInfo {
    ...
    [data writeToFile:[GlodBuleHTUserManager taouserInfoPath] atomically:YES];
...
+ (void)tao_doUserLogin {
    [GlodBuleHTLoginAlertView taoshowLoginAlertViewWithSelectBlock:^(HTLoginPlatform platform) {
        GlodBuleHTUserManager *manager = [GlodBuleHTUserManager manager];
...
+ (void)tao_doUserLogout {
    ...
    [GlodBuleDRSandBoxManager taodeleteFileAtPath:[self taouserInfoPath] doneBlock:^(NSString * _Nonnull filePath, BOOL success, NSError * _Nonnull error) {
        BJLog(@"用戶信息刪除 %d", success);
    }];
    [[NSNotificationCenter defaultCenter] postNotificationName:kUserLogStatusChagneNotice object:nil];
    [[GlodBuleHTUserManager manager].tao_fbLoginManager logOut];
}
...
+ (void)tao_refreshUserInfoWithSuccessBlock:(dispatch_block_t)block {
    [GlodBuleHTUserRequest taorequestUserInfoWithSuccessBlock:^(NSDictionary * _Nonnull userInfo) {
...
+ (NSString *)taouserInfoPath {
    return [NSString stringWithFormat:@"%@/userInfo.json", [GlodBuleDRSandBoxManager taogetDocumentPath]];
}
