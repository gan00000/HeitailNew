...
@implementation UUaksDRSandBoxManager
#pragma mark - 磁盘信息获取
...
+ (void)taogetDirectoryInDocumentWithName:(NSString *)dirName
    ...
    NSString *document = [self taogetDocumentPath];
...
+ (void)taogetFilePathWithName:(NSString *)fileName
    ...
    [self taogetDirectoryInDocumentWithName:dirName doneBlock:^(BOOL success, NSError *error, NSString *dirPath) {
        if (!success) {
            block(error, nil);
        } else {
