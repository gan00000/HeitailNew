...
@interface FFlaliHTUserInfoEditViewController ()
...
@end
@implementation FFlaliHTUserInfoEditViewController
+ (instancetype)taoviewController {
    return [[FFlaliHTUserInfoEditViewController alloc] init];
}
- (void)updateUserInfo {
...
- (IBAction)avatarSelectAction:(UIButton *)sender {
    ...
    if (status == PHAuthorizationStatusAuthorized) {
        ...
        [NSNiceHTUserManager taophotoAlbumDenied];
    } else {
        kWeakSelf
...
- (void)saveUserInfo {
    [NSNiceBJLoadingHud showHUDWithText:@"正在保存" inView:self.navigationController.view];
    NSString *email;
    NSString *userName;
    ...
    kWeakSelf
    [TuTuosHTUserRequest taoupdateUserInfoWithEmail:email displayName:userName file:base64Avatar successBlock:^(NSDictionary * _Nonnull userInfo) {
        
        [NSNiceHTUserManager tao_refreshUserInfoWithSuccessBlock:^{
            [NSNiceBJLoadingHud hideHUDInView:weakSelf.navigationController.view];
