...
#import "YeYeeHTRankLeftCell.h"
#import "YeYeeHTRankRightCell.h"
@interface KSasxHTRankHomeCell () <UITableViewDelegate, UITableViewDataSource>
@property (weak, nonatomic) IBOutlet UILabel *titleLabel;
@property (weak, nonatomic) IBOutlet UITableView *leftTableView;
//@property (weak, nonatomic) IBOutlet UIScrollView *scrollContentView;
//@property (nonatomic, strong) UITableView *rightTableView;
@property (nonatomic, weak) NSArray<CfipyHTRankModel *> *rankList;
@end
@implementation KSasxHTRankHomeCell
- (void)awakeFromNib {
    [super awakeFromNib];
...
- (void)taosetupWithTitle:(NSString *)title rankList:(NSArray<CfipyHTRankModel *> *)rankList {
...
- (void)setupLeftTableView {
    ...
    [self.leftTableView registerNib:[UINib nibWithNibName:NSStringFromClass([YeYeeHTRankLeftCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([YeYeeHTRankLeftCell class])];
}
/**
- (void)setupRightTableView {
    ...
    [self.rightTableView registerNib:[UINib nibWithNibName:NSStringFromClass([YeYeeHTRankRightCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([YeYeeHTRankRightCell class])];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CfipyHTRankModel *model = self.rankList[indexPath.row];
    if (tableView == self.leftTableView) {
        YeYeeHTRankLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([YeYeeHTRankLeftCell class])];
        [cell taorefreshWithRankModel:model row:indexPath.row];
        return cell;
    }
    YeYeeHTRankRightCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([YeYeeHTRankRightCell class])];
    [cell taorefreshWithRankModel:model row:indexPath.row];
    return cell;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    NSArray *titles = @[@"隊名",@"勝-負", @"勝率", @"勝差", @"連勝/負"];
    
    if (tableView == self.leftTableView) {
        [self addLabelToView:view withFrame:CGRectMake(0, 0, 120, 40) text:@"排名"];
        //[self addLabelToView:view withFrame:CGRectMake(50, 0, 80, 30) text:@"隊名"];
        for (NSInteger i=0; i<4; i++) {
            [self addLabelToView:view withFrame:CGRectMake(i*60 + 120, 0, 60, 40) text:titles[i + 1]];
        }
    }
//    else {
