...
@interface YeYeeHTRankHomeViewController () <UIScrollViewDelegate>
...
@end
@implementation YeYeeHTRankHomeViewController
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    UIViewController *vc;
    if (index == 0) {
        WSKggHTRankEastWestViewController *ewVc = [WSKggHTRankEastWestViewController taoviewController];
        vc = ewVc;
    } else if (index == 1) {
        BlysaHTRankZoneViewController *zoneVc = [BlysaHTRankZoneViewController taoviewController];
        vc = zoneVc;
    }
