...
#import "KMonkeyHTSearchHistoryTableViewCell.h"
#import "KMonkeySearchRequest.h"
#import "RRDogSearchData.h"
#import "HourseHTNewsDetailViewController.h"

@interface MMTodaySearchViewController () <UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic,strong) KMonkeySearchRequest *xSearchRequest;
@property (nonatomic, strong) RRDogSearchData *xSearchData;

@end

@implementation MMTodaySearchViewController
...
- (void)viewDidLoad {
    ...
    [self setupViews];
    self.xSearchRequest = [[KMonkeySearchRequest alloc] init];
...
- (void)setupViews {
   ...
   // self.title = @"搜索";
    ...
    [self.historyTableView registerNib:[UINib nibWithNibName:@"KMonkeyHTSearchHistoryTableViewCell" bundle:nil]
            forCellReuseIdentifier:@"KMonkeyHTSearchHistoryTableViewCell"];
    ...
    NSArray<RRDogAccountModel *> *historiesSearch = [[PXFunConfigCoreUtil share] getAccountModels];
...
- (void)setupContentTableView {
    ...
    self.xSearchData = [[RRDogSearchData alloc] init];
    ...
    [self.searchContentTableView registerNib:[UINib nibWithNibName:@"YYPackageHTNewsHomeCell" bundle:nil]
         forCellReuseIdentifier:@"YYPackageHTNewsHomeCell"];
    kWeakSelf
//    self.searchContentTableView.mj_header = [HourseMJRefreshGenerator bj_headerWithRefreshingBlock:^{
//        weakSelf.searchContentTableView.mj_footer.hidden = YES;
//        [weakSelf loadData:@""];
//    }];
    self.searchContentTableView.mj_footer = [HourseMJRefreshGenerator bj_foorterWithRefreshingBlock:^{
    ...
    self.xSearchData.xClickHander = ^(NSInteger i) {
        ...
        PXFunHTNewsModel *newsModel = self.newsList[i];
...
- (void)loadData:(NSString *)key {
    ...
    kWeakSelf
    [self.xSearchRequest requestWithKey:key successBlock:^(NSArray<PXFunHTNewsModel *> *newsList) {
        weakSelf.newsList = newsList;
        weakSelf.newsRequestDone = YES;
        
        [[PXFunConfigCoreUtil share]saveAccount:key password:@"1" updateTime:YES];
...
- (void)loadNextPage {
    self.newsRequestDone = NO;
    kWeakSelf
    [self.xSearchRequest loadNextPageWithSuccessBlock:^(NSArray<PXFunHTNewsModel *> *newsList) {
        weakSelf.newsList = newsList;
        weakSelf.newsRequestDone = YES;
        [weakSelf refreshUI];
     } errorBlock:^(SundayBJError *error) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RRDogAccountModel *data = self.searchHistoryArray[indexPath.row];
    
    KMonkeyHTSearchHistoryTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"KMonkeyHTSearchHistoryTableViewCell"];
    [cell setHistory:data.accountName];
    cell.mClickHander = ^(NSInteger i) {
        [[PXFunConfigCoreUtil share] removeAccount:data.accountName];
        [self refreshHistoryData];
    };
    return cell;
}
