//
//  GlodBuleSearchViewController.m
//  SkyBallPlay
//
...
#import "GlodBuleHTSearchHistoryTableViewCell.h"
#import "GlodBuleSearchRequest.h"
#import "GlodBuleSearchData.h"
#import "GlodBuleHTNewsDetailViewController.h"

@interface GlodBuleSearchViewController () <UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic,strong) GlodBuleSearchRequest *xSearchRequest;
@property (nonatomic, strong) GlodBuleSearchData *xSearchData;

@end

@implementation GlodBuleSearchViewController
...
- (void)viewDidLoad {
    ...
    [self setupViews];
    self.xSearchRequest = [[GlodBuleSearchRequest alloc] init];
...
- (void)setupViews {
   ...
   // self.title = @"搜索";
    ...
    [self.historyTableView registerNib:[UINib nibWithNibName:@"GlodBuleHTSearchHistoryTableViewCell" bundle:nil]
            forCellReuseIdentifier:@"GlodBuleHTSearchHistoryTableViewCell"];
    ...
    NSArray<GlodBuleAccountModel *> *historiesSearch = [[GlodBuleConfigCoreUtil share] getAccountModels];
...
- (void)setupContentTableView {
    ...
    self.xSearchData = [[GlodBuleSearchData alloc] init];
    ...
    [self.searchContentTableView registerNib:[UINib nibWithNibName:@"GlodBuleHTNewsHomeCell" bundle:nil]
         forCellReuseIdentifier:@"GlodBuleHTNewsHomeCell"];
    kWeakSelf
//    self.searchContentTableView.mj_header = [GlodBuleMJRefreshGenerator bj_headerWithRefreshingBlock:^{
//        weakSelf.searchContentTableView.mj_footer.hidden = YES;
//        [weakSelf loadData:@""];
//    }];
    self.searchContentTableView.mj_footer = [GlodBuleMJRefreshGenerator bj_foorterWithRefreshingBlock:^{
    ...
    self.xSearchData.xClickHander = ^(NSInteger i) {
        ...
        GlodBuleHTNewsModel *newsModel = self.newsList[i];
...
- (void)loadData:(NSString *)key {
    ...
    kWeakSelf
    [self.xSearchRequest requestWithKey:key successBlock:^(NSArray<GlodBuleHTNewsModel *> *newsList) {
        weakSelf.newsList = newsList;
        weakSelf.newsRequestDone = YES;
        
        [[GlodBuleConfigCoreUtil share]saveAccount:key password:@"1" updateTime:YES];
...
- (void)loadNextPage {
    self.newsRequestDone = NO;
    kWeakSelf
    [self.xSearchRequest loadNextPageWithSuccessBlock:^(NSArray<GlodBuleHTNewsModel *> *newsList) {
        weakSelf.newsList = newsList;
        weakSelf.newsRequestDone = YES;
        [weakSelf refreshUI];
     } errorBlock:^(GlodBuleBJError *error) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GlodBuleAccountModel *data = self.searchHistoryArray[indexPath.row];
    
    GlodBuleHTSearchHistoryTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"GlodBuleHTSearchHistoryTableViewCell"];
    [cell setHistory:data.accountName];
    cell.mClickHander = ^(NSInteger i) {
        [[GlodBuleConfigCoreUtil share] removeAccount:data.accountName];
        [self refreshHistoryData];
    };
    return cell;
}
