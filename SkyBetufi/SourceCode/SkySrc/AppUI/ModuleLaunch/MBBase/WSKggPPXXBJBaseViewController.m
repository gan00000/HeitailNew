...
@interface WSKggPPXXBJBaseViewController ()
@end
@implementation WSKggPPXXBJBaseViewController
...
- (void)viewDidLoad {
    ...
    if ([self isKindOfClass:[FFlaliPPXXBJLaunchViewController class]]) {
    ...
    if (self.navigationController.viewControllers.count == 1 && ![self isKindOfClass:[TuTuosHTMeHomeViewController class]] && ![self isKindOfClass:[KSasxHTHomeLeftViewController class]]) {
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if ([currentClassString isEqualToString:NSStringFromClass([NSNiceHTNewMatchHomeViewController class])]) {
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([BlysaHTNewsHomeViewController class])]){
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([TuTuosHTFilmHomeViewController class])]){
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([YeYeeHTDataHomeViewController class])]){
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([YeYeeHTRankHomeViewController class])]){
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([KSasxHTTabBarHomeViewController class])]){
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([TuTuosHTMeHomeViewController class])]){
    ...
    }else if ([currentClassString isEqualToString:NSStringFromClass([FFlaliHTMainPageViewController class])]){
...
- (void)showMeCenter {
    ...
    TuTuosHTMeHomeViewController *meVc = [TuTuosHTMeHomeViewController taoviewController];
    YeYeePPXXBJNavigationController *nav = [[YeYeePPXXBJNavigationController alloc] initWithRootViewController:meVc];
    [nav.navigationBar lt_setBackgroundColor:appBaseColor];
    [self presentViewController:nav animated:YES completion:nil];
}

- (void)showSearchViewController {
    UUaksSearchViewController *meVc = [UUaksSearchViewController taoviewController];
//    YeYeePPXXBJNavigationController *nav = [[YeYeePPXXBJNavigationController alloc] initWithRootViewController:meVc];
...
//        UIImage *userImage = [NSNiceHTUserManager tao_userInfo].avatar;
//        [self.taomeCenterButton setImage:[WSKggPPXXBJBaseViewController taofixImageSize:userImage toSize:CGSizeMake(36, 36)] forState:UIControlStateNormal];
        
        [self.taomeCenterButton sd_setImageWithURL:[NSURL URLWithString:[NSNiceHTUserManager tao_userInfo].user_img] forState:UIControlStateNormal placeholderImage:HT_DEFAULT_AVATAR_LOGO];
    } else {
...
- (void)setUpRightSearchButton {
    ...
    NSString *currentClassString = NSStringFromClass([self class]);
    if ([currentClassString isEqualToString:NSStringFromClass([BlysaHTNewsHomeViewController class])]){
        [self.rightSearchButton setImage:[UIImage imageNamed:@"icon_search"] forState:UIControlStateNormal];

    }else if ([currentClassString isEqualToString:NSStringFromClass([TuTuosHTFilmHomeViewController class])]){
        
        [self.rightSearchButton setImage:[UIImage imageNamed:@"icon_search"] forState:UIControlStateNormal];
    }else if (isAppInView && [currentClassString isEqualToString:NSStringFromClass([FFlaliHTMainPageViewController class])]){
