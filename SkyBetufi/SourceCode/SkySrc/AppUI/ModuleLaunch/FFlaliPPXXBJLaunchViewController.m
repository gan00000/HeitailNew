...
#import "BByasDHGuidePageHUD.h"
#import "NSDate+MMoogKMonkeyCore.h"
...
@interface FFlaliPPXXBJLaunchViewController () <CAAnimationDelegate>
@property (nonatomic, strong) BlysaPPXXBJMainViewController *tabBarController;
//@property (nonatomic,strong) MMDrawerController *drawerController;

@property (nonatomic, assign) NSInteger tryTimes;
@property (nonatomic, assign) BOOL needGuidePage;
@end
@implementation FFlaliPPXXBJLaunchViewController
...
- (void)handleAppInViewResult:(NSDictionary *)configDictionary {
    if (configDictionary) {
        ...
        if ([appVersion isEqualToString:version] && [examine isEqualToString:@"1"]) {
            [NSNiceHTUserManager manager].appInView = YES;
            BJLog(@"getRequestCommon in view");
        }else{
            BJLog(@"getRequestCommon not in view");
            [NSNiceHTUserManager manager].appInView = NO;
        }
        
        [NSNiceHTUserManager manager].showTextLive = YES;
    }
...
//    }];
    ...
    [BlysaBJHTTPServiceEngine tao_getRequestCommon:url params:nil successBlock:^(id responseData) {
...
-(void) checkUpdate
{
    [TuTuosHTUserRequest taodoCheckAppUpdateWithSuccessBlock:^(MMoogHTUpdateInfoModel * _Nonnull kHTUpdateInfoModel) {
        
        if (kHTUpdateInfoModel && !kHTUpdateInfoModel.content) {
            
            [UUaksHTShowUpdateInfoView showWithData:kHTUpdateInfoModel];
...
#pragma mark - getters
- (BlysaPPXXBJMainViewController *)tabBarController {
    if (!_tabBarController) {
        _tabBarController = [[BlysaPPXXBJMainViewController alloc] init];
    }
    [NSNiceHTUserManager manager].mainTabBarController = _tabBarController;
...
//    [leftSideNavController setRestorationIdentifier:@"MMExampleLeftNavigationControllerRestorationKey"];
    ...
    KSasxHTHomeLeftViewController *meHomeViewController = [KSasxHTHomeLeftViewController taoviewController];
    YeYeePPXXBJNavigationController *leftNavController = [[YeYeePPXXBJNavigationController alloc] initWithRootViewController:meHomeViewController];
    
//    YeYeePPXXBJNavigationController *centerNavController = [[YeYeePPXXBJNavigationController alloc] initWithRootViewController:self.tabBarController];
...
- (void)taosetStaticGuidePage {
    ...
    BByasDHGuidePageHUD *guidePage = [[BByasDHGuidePageHUD alloc] dh_initWithFrame:self.view.frame imageNameArray:imageNameArray buttonIsHidden:NO];
...
- (void)taosetDynamicGuidePage {
    ...
    BByasDHGuidePageHUD *guidePage = [[BByasDHGuidePageHUD alloc] dh_initWithFrame:self.view.frame imageNameArray:imageNameArray buttonIsHidden:YES];
    guidePage.slideInto = YES;
    [self.navigationController.view addSubview:guidePage];
}
...
//    BByasDHGuidePageHUD *guidePage = [[BByasDHGuidePageHUD alloc] dh_initWithFrame:self.view.bounds videoURL:videoURL];
//    [self.navigationController.view addSubview:guidePage];
}
