...
@interface MMTodayHTDataHomeViewController () <UIScrollViewDelegate>
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        GGCatHTRankEastWestViewController *vc = [GGCatHTRankEastWestViewController taoviewController];
        //vc.type = index + 1;
    ...
    HourseHTDataHomeSubViewController *vc = [HourseHTDataHomeSubViewController taoviewController];
    if (index == 0) {
        vc.type = 1;//球员1
    } else if(index == 2){
         vc.type = 2;//球队2
