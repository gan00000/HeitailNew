...
@interface BlysaHTDataHomeViewController () <UIScrollViewDelegate>
...
- (void)loadChildViewControllerByIndex:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        KSasxHTRankEastWestViewController *vc = [KSasxHTRankEastWestViewController taoviewController];
        //vc.type = index + 1;
    ...
    BByasHTDataHomeSubViewController *vc = [BByasHTDataHomeSubViewController taoviewController];
    if (index == 0) {
        vc.type = 1;//球员1
    } else if(index == 2){
         vc.type = 2;//球队2
