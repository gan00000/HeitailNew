...
#import "GGCatHTDataPlayerRightCell.h"
#import "KMonkeyHTDataTeamRightCell.h"
#import "GGCatHTDataAllRankRequest.h"
@interface YYPackageHTDataMoreViewController () <UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong) NSArray<NDeskHTDataPlayerRankModel *> *playerList;
@property (nonatomic, strong) NSArray<HourseHTDataTeamRankModel *> *teamList;
...
- (void)loadData {
    if (self.type == 1) {
        [GGCatHTDataAllRankRequest requestAllPlayerRankDataWithType:self.subType successBlock:^(NSArray<NDeskHTDataPlayerRankModel *> *allPlayerRankList) {
        ...
        [GGCatHTDataAllRankRequest taorequestAllTeamRankDataWithType:self.subType successBlock:^(NSArray<HourseHTDataTeamRankModel *> *allTeamRankList) {
...
- (void)setupLeftTableView {
    ...
    if (self.type == 1) {
        [self.leftTableView registerNib:[UINib nibWithNibName:NSStringFromClass([KMonkeyHTDataPlayerLeftCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([KMonkeyHTDataPlayerLeftCell class])];
    } else {
        [self.leftTableView registerNib:[UINib nibWithNibName:NSStringFromClass([RRDogHTDataTeamLeftCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([RRDogHTDataTeamLeftCell class])];
    }
}
- (void)setupRightTableView {
    ...
    if (self.type == 1) {
        [self.rightTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GGCatHTDataPlayerRightCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([GGCatHTDataPlayerRightCell class])];
    } else {
        [self.rightTableView registerNib:[UINib nibWithNibName:NSStringFromClass([KMonkeyHTDataTeamRightCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([KMonkeyHTDataTeamRightCell class])];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.type == 1) {
        NDeskHTDataPlayerRankModel *playerModel = self.playerList[indexPath.row];
        if (tableView == self.leftTableView) {
            KMonkeyHTDataPlayerLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([KMonkeyHTDataPlayerLeftCell class])];
            [cell taorefreshWithPlayerModel:playerModel row:indexPath.row];
            return cell;
        }
        GGCatHTDataPlayerRightCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([GGCatHTDataPlayerRightCell class])];
        [cell taorefreshWithPlayerModel:playerModel row:indexPath.row];
        return cell;
    } else {
        HourseHTDataTeamRankModel *teamModel = self.teamList[indexPath.row];
        if (tableView == self.leftTableView) {
            RRDogHTDataTeamLeftCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([RRDogHTDataTeamLeftCell class])];
            [cell taorefreshWithTeamModel:teamModel row:indexPath.row];
            return cell;
        }
        KMonkeyHTDataTeamRightCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([KMonkeyHTDataTeamRightCell class])];
        [cell taorefreshWithTeamModel:teamModel row:indexPath.row];
        return cell;
    }
}
