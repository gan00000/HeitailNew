...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    ...
    [JSStringM appendString:@"document.getElementsByClassName('ytp-pause-overlay ytp-scroll-min')[0].style.visibility = 'hidden';"];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([navigationAction.request.URL.host isEqualToString: self.originURL.host]) {
        decisionHandler(WKNavigationActionPolicyAllow);
    }else if ([navigationAction.request.URL.scheme isEqualToString:@"ytplayer"]) {
        [self notifyDelegateOfYouTubeCallbackUrl:navigationAction.request.URL];
        decisionHandler(WKNavigationActionPolicyCancel);
    }else if ([navigationAction.request.URL.scheme isEqual: @"http"] || [navigationAction.request.URL.scheme isEqual:@"https"]){
        if ([self handleHttpNavigationToUrl:navigationAction.request.URL]) {
            decisionHandler(WKNavigationActionPolicyAllow);
        }else{
            decisionHandler(WKNavigationActionPolicyCancel);
        }
...
//    NSLog(@"embedHTML=%@,originURL=%@",embedHTML,self.originURL);
