...
#import "FFlaliXJEmptyView.h"
#import "Masonry.h"
#import "AFNetworkReachabilityManager.h"
static void * kEmptyViewKey;
@implementation UIView (CfipyEmptyView)
- (void)setEmptyView:(FFlaliXJEmptyView *)emptyView {
    objc_setAssociatedObject(self, &kEmptyViewKey, emptyView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (FFlaliXJEmptyView *)emptyView {
...
- (void)showEmptyViewWithImage:(UIImage *)image title:(NSString *)title tapBlock:(dispatch_block_t)tapBlock edge:(UIEdgeInsets)edge {
    [self hideEmptyView];
    FFlaliXJEmptyView *emptyView = [[NSBundle mainBundle] loadNibNamed:@"FFlaliXJEmptyView" owner:nil options:nil].firstObject;
    if (title.length > 0) {
    ...
    if ([AFNetworkReachabilityManager sharedManager].networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        emptyView.imageView.image = [UIImage imageNamed:@"gurk_status_icon_no_network"];
