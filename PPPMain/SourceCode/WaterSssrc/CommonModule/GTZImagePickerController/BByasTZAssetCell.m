...
@end
@implementation BByasTZAssetCell
- (void)setModel:(UUaksTZAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[WSKggTZImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[WSKggTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[WSKggTZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)fetchBigImage {
    _bigImageRequestID = [[WSKggTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (CfipyTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[CfipyTZProgressView alloc] init];
...
- (void)setModel:(TZAlbumModel *)model {
    ...
    [[WSKggTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
