...
@interface BByasTZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
@end
@implementation BByasTZImagePickerController
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[BByasTZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[BByasTZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (![[WSKggTZImageManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    WSKggTZPhotoPreviewController *previewVc = [[WSKggTZPhotoPreviewController alloc] init];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        WSKggTZPhotoPickerController *photoPickerVc = [[WSKggTZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[WSKggTZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(TZAlbumModel *model) {
...
- (void)setPickerDelegate:(id<TZImagePickerControllerDelegate>)pickerDelegate {
    ...
    [WSKggTZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [WSKggTZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}
- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [WSKggTZImageManager manager].photoWidth = photoWidth;
}
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    _selectedAssets = selectedAssets;
    _selectedModels = [NSMutableArray array];
    for (id asset in selectedAssets) {
        UUaksTZAssetModel *model = [UUaksTZAssetModel modelWithAsset:asset type:[[WSKggTZImageManager manager] getAssetType:asset]];
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [WSKggTZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)configTableView {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        BByasTZImagePickerController *imagePickerVc = (BByasTZImagePickerController *)self.navigationController;
        [[WSKggTZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
