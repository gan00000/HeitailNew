...
@interface UUaksDHGifImageOperation ()
{
...
@implementation UUaksDHGifImageOperation
...
- (void)play {
    if (count > 0) {
        ...
        dispatch_once(&onceToken, ^{
            NSLog(@"[UUaksDHGifImageOperation]:请检测网络或者http协议");
        });
    }
}
...
/**< 使用案例: [self.XXX addSubview:[[UUaksDHGifImageOperation alloc] initWithFrame:self.adFrame gifImageName:@"XXX.gif"]]; */
...
- (void)activiTap:(UITapGestureRecognizer*)recognizer{
    NSLog(@"[UUaksDHGifImageOperation.h]:activiTap:recognizer");
}
@end
