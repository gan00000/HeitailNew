#import "BByasHTDataHomeSubViewController.h"
...
@interface BByasHTDataHomeSubViewController () <UITableViewDelegate, UITableViewDataSource>
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic, strong) TuTuosHTDataHomeInfoModel *homeInfoModel;
@property (nonatomic, strong) WSKggBJError *error;
@end
@implementation BByasHTDataHomeSubViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BByasHTDataHomeHeaderCell *headerCell;
    UUaksHTDataHomePlayerCell *playerCell;
    MMoogHTDataHomeTeamCell *teamCell;
    if (indexPath.row == 0) {
        headerCell = [tableView dequeueReusableCellWithIdentifier:@"BByasHTDataHomeHeaderCell"];
    } else {
        if (self.type == 1) {
            playerCell = [tableView dequeueReusableCellWithIdentifier:@"UUaksHTDataHomePlayerCell"];
        } else {
            teamCell = [tableView dequeueReusableCellWithIdentifier:@"MMoogHTDataHomeTeamCell"];
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CfipyHTDataMoreViewController *moreVc = [CfipyHTDataMoreViewController taoviewController];
...
- (void)setupViews {
    ...
    if (self.type == 1) {
        [self.tableView registerNib:[UINib nibWithNibName:@"UUaksHTDataHomePlayerCell" bundle:nil]
             forCellReuseIdentifier:@"UUaksHTDataHomePlayerCell"];
    } else {
        [self.tableView registerNib:[UINib nibWithNibName:@"MMoogHTDataHomeTeamCell" bundle:nil]
             forCellReuseIdentifier:@"MMoogHTDataHomeTeamCell"];
    }
    [self.tableView registerNib:[UINib nibWithNibName:@"BByasHTDataHomeHeaderCell" bundle:nil]
         forCellReuseIdentifier:@"BByasHTDataHomeHeaderCell"];
    kWeakSelf
...
- (void)loadData {
    [BByasHTDataHomeRequest taorequestWithType:self.type successBlock:^(TuTuosHTDataHomeInfoModel *infoModel) {
