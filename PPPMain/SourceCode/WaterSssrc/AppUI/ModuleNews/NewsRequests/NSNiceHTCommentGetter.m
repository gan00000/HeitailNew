...
@interface NSNiceHTCommentGetter ()
...
@end
@implementation NSNiceHTCommentGetter
...
- (void)loadHotCommentsWithCompleteBlock:(dispatch_block_t)block {
    [FFlaliHTNewsAdditionRequest taorequestHotCommentWithOffset:self.hotOffset newsId:self.post_id successBlock:^(NSArray<WSKggHTCommentModel *> * _Nonnull commentList, NSInteger pages) {
        if (self.hotOffset == 0) {
        ...
        [[CfipyPPXXBJViewControllerCenter currentViewController].view showToast:@"獲取「熱門評論」失敗"];
        if (block) {
            block();
        }
    }];
...
- (void)loadNormalCommentsWithCompleteBlock:(dispatch_block_t)block {
    [FFlaliHTNewsAdditionRequest taorequestNormalCommentWithOffset:self.normalOffset newsId:self.post_id successBlock:^(NSArray<WSKggHTCommentModel *> * _Nonnull commentList, NSInteger pages) {
        ...
        [[CfipyPPXXBJViewControllerCenter currentViewController].view showToast:@"獲取「全部評論」失敗"];
        if (block) {
            block();
        }
    }];
}
- (void)countHeightForComments:(NSArray<WSKggHTCommentModel *> *)comments {
    for (WSKggHTCommentModel *commentModel in comments) {
