...
@interface WSKggHTUserInfoEditViewController ()
...
@end
@implementation WSKggHTUserInfoEditViewController
+ (instancetype)taoviewController {
    return [[WSKggHTUserInfoEditViewController alloc] init];
}
- (void)updateUserInfo {
...
- (IBAction)avatarSelectAction:(UIButton *)sender {
    ...
    if (status == PHAuthorizationStatusAuthorized) {
        ...
        [TuTuosHTUserManager taophotoAlbumDenied];
    } else {
        kWeakSelf
...
- (void)saveUserInfo {
    [UUaksBJLoadingHud showHUDWithText:@"正在保存" inView:self.navigationController.view];
    NSString *email;
    NSString *userName;
    ...
    kWeakSelf
    [NSNiceHTUserRequest taoupdateUserInfoWithEmail:email displayName:userName file:base64Avatar successBlock:^(NSDictionary * _Nonnull userInfo) {
        
        [TuTuosHTUserManager tao_refreshUserInfoWithSuccessBlock:^{
            [UUaksBJLoadingHud hideHUDInView:weakSelf.navigationController.view];
