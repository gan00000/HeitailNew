...
@interface FFlaliHTCommentViewController () <UITableViewDataSource, UITableViewDelegate>
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic, strong) NSMutableArray<CfipyHTNewsModel *> *dataSource;
@property (nonatomic, assign) NSInteger offset;
@end
@implementation FFlaliHTCommentViewController
+ (instancetype)taoviewController {
    return [[FFlaliHTCommentViewController alloc] init];
}
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"我的評論";
...
- (void)loadData {
    [NSNiceHTUserRequest taorequestMyCommentWithOffset:self.offset successBlock:^(NSArray<CfipyHTNewsModel *> * _Nonnull newsList, NSInteger pages) {
        for (CfipyHTNewsModel *model in newsList) {
...
- (void)setupTableView {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"KSasxHTMyCommentCell" bundle:nil]
         forCellReuseIdentifier:@"KSasxHTMyCommentCell"];
    kWeakSelf
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    KSasxHTMyCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:@"KSasxHTMyCommentCell"];
