...
@interface TuTuosHTWordLive2Request ()
@property (nonatomic, strong) NSMutableArray *wordLiveList;
@property (nonatomic, assign) NSInteger page;
@end
@implementation TuTuosHTWordLive2Request
...
- (void)getWordLiveFeedWithGameId:(NSString *)game_id
    ...
    [NSNiceBJHTTPServiceEngine tao_getRequestWithFunctionPath:API_MATCH_LIVE_FEED_PAGING params:params successBlock:^(id responseData) {
        if (isFirst) {
...
//            }
        ...
        for (FFlaliHTMatchLiveFeedModel *m in self.wordLiveList) {
//            m.toCount = toCount;
            if (currentModel) {
                //判断是否得分
