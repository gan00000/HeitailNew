...
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
    _mLineChartUtil = [[BlysaLineChartUtil alloc]init];
...
- (void)setMatchSummaryModel:(KSasxHTMatchSummaryModel *)summaryModel liveFeedModel:(NSArray<FFlaliHTMatchLiveFeedModel *> *)liveFeedModel
{
    ...
    NSArray<FFlaliHTMatchLiveFeedModel *> *c_liveFeedModel = [liveFeedModel copy];
    ...
    int yMax = 0;
    int tempSeconds = 0;
    for (FFlaliHTMatchLiveFeedModel *feedMode in c_liveFeedModel) {
...
//        BJLog(@"quarter = %ld",quarter);
    ...
    FFlaliHTMatchLiveFeedModel *last_feedMode = c_liveFeedModel.lastObject;
